...
type LumosSprite = (typeof LUMOS_SPRITES)[number];
...
function spriteSize(sprite: LumosSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = LUMOS_SPRITES[(baseCol + baseRow) % LUMOS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = LUMOS_SPRITES[Math.floor(Math.random() * LUMOS_SPRITES.length)]!;
