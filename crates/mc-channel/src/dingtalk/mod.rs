...
//! 本文件的**唯一**内容是"这个平台在这里注册工厂"的位置声明；实现归 M7-7 / M7-8 / M7-9
//! （`docs/60` §3.3 的写集表：本目录下的每个子文件都有**一个**写者）。
//!
//! # 这个平台的面（实现者按这三条认领自己的文件）
//!
//! - Stream WebSocket（每个 BYO installation 一条）；
//! - 7 条路由（含 agent 级 `/api/agents/{id}/dingtalk/groups`）；
//! - 引用卡片 / 互动卡片 / 媒体出站；`group-routes` **必须保持 404**；
//!
...
//! M7-7 / M7-8 / M7-9 的事。anchor 的 `register()` 是**空实现**，因此注册表在 anchor 期是空的
...
pub fn register(_registry: &Registry, _deps: &ChannelDeps) {
    // M7 的 M7-7 / M7-8 / M7-9 在这里 `registry.register(ChannelKind::DingTalk, factory)`。
}
