...
//! 本文件的**唯一**内容是"这个平台在这里注册工厂"的位置声明；实现归 M7-3 / M7-4
//! （`docs/60` §3.3 的写集表：本目录下的每个子文件都有**一个**写者）。
//!
//! # 这个平台的面（实现者按这三条认领自己的文件）
//!
//! - `socket_mode` 单条部署级连接（`xapp-` app token）；
//! - BYO 安装（4 条 workspace 路由 + `/api/slack/binding/redeem`）；
//! - Block Kit 出站与回复投递；
//!
...
//! M7-3 / M7-4 的事。anchor 的 `register()` 是**空实现**，因此注册表在 anchor 期是空的
...
pub fn register(_registry: &Registry, _deps: &ChannelDeps) {
    // M7 的 M7-3 / M7-4 在这里 `registry.register(ChannelKind::Slack, factory)`。
}
