//! Slack 的用户绑定令牌流程：**铸**一枚单次令牌 → 用户在 web app 里**兑换** →
//! 平台用户 id 绑到 Multica 账号
//! （上游 `internal/integrations/slack/binding.go`，165 行）。
//!
//! - **写者**：M7-4（`docs/60-M7-PLAN.md` §3.3）。
//! - **上游定位**：未绑定的 Slack 用户给 bot 发消息 ⇒ 出站回复器（`replier.rs`）铸一枚
//!   令牌并回「点这里绑定」；用户点进 `AppURL + /slack/bind?token=…`；`POST
//!   /api/slack/binding/redeem` 把令牌里的 Slack user id 绑到**会话身份**（不是令牌里的）。
//! - **走泛化表**：`channel_binding_token` / `channel_user_binding`，`channel_type='slack'`
//!   （上游注释逐字：lark 的 `ChannelStore` 把 `feishu` 写死，所以不能复用它的服务）。
//!
//! # 三条安全判据（逐条可测）
//!
...
//! 3. **兑换是原子的**：消费令牌、校验成员资格、插绑定行**同一事务**提交 ⇒
//!    非成员的一次尝试**不烧掉**令牌（上游注释逐字：`returning before Commit rolls
//!    the consume back`）。
//!
//! # 与上游的一处形态差异（登记 `docs/32` §15）
//!
//! 上游的 `RedeemAndBind` 自己持有 `pgx.Tx` 并逐条驱动 SQL；本仓把「一个事务里的三段」
//! 收进 [`BindingStore::redeem_and_bind`] 这**一个端口方法**（层次铁律：channel 层不碰 SQL）。
//! ⇒ 业务判决（三种错误、成员闸门）仍在**本文件**，但「原子性」由实现保证；
//! 实现必须逐字照上游的顺序：consume → membership → insert，失败即回滚。
//!
//! # 随机性（登记 `docs/32` §15）
//!
...
///
/// `redeem` 的判决由 [`RedeemOutcome`] 表达（那是**产品**结果）；本枚举给铸令牌与
/// 存储故障用。
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum BindingError {
    /// 令牌不存在 / 已消费 / 已过期（**同一个**不透明错误，见模块文档判据 2）。
    #[error("slack: binding token invalid or expired")]
    TokenInvalid,
    /// 这个 Slack user id 已绑到**另一个** Multica 用户（转移必须显式解绑）。
    ...
    #[error("slack: binding store failure: {message}")]
...
impl BindingError {
    ...
    pub fn code(&self) -> &'static str {
        match self {
            Self::TokenInvalid => "slack_binding_token_invalid",
            Self::AlreadyAssigned => "slack_binding_already_assigned",
            Self::NotMember => "slack_binding_not_member",
            Self::Store { .. } => "slack_binding_store_error",
        }
    }
}
...
pub struct RedeemedBinding {
    ...
    /// 令牌里的 Slack user id（**不是**兑换者的 —— 那来自会话）。
...
/// consume → membership → insert 的顺序跑（见模块文档的形态差异）。
#[async_trait]
...
impl BindingTokenService {
    ...
    ///
    /// **幂等**（本片的专属验收）：重复兑换同一枚令牌 ⇒ 第二次是
    /// [`RedeemOutcome::TokenInvalid`]（令牌已被消费），**不会**重复插
    /// `channel_user_binding` 行 —— 单次性由 `consumed_at` 的 `UPDATE … WHERE
    /// consumed_at IS NULL` 保证，而不是靠应用层读改（那样两个并发请求都能通过）。
    pub async fn redeem(
        &self,
        raw: &str,
        multica_user_id: Id,
