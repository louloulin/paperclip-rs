...
//! 本文件的**唯一**内容是"这个平台在这里注册工厂"的位置声明；实现归 M7-5 / M7-6
//! （`docs/60` §3.3 的写集表：本目录下的每个子文件都有**一个**写者）。
//!
//! # 这个平台的面（实现者按这三条认领自己的文件）
//!
//! - `getUpdates` 长轮询（50s server-side hold）；
//! - 4 条安装路由 + `/api/telegram/binding/redeem`；
//! - 出站发送 / 投递状态机 / Markdown 转换；
//!
...
//! M7-5 / M7-6 的事。anchor 的 `register()` 是**空实现**，因此注册表在 anchor 期是空的
...
pub fn register(_registry: &Registry, _deps: &ChannelDeps) {
    // M7 的 M7-5 / M7-6 在这里 `registry.register(ChannelKind::Telegram, factory)`。
}
