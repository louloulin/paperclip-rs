...
#[tokio::test]
async fn noop_ok() {
    let e = ToolHookEvent::Deleted {
        company_id: Uuid::new_v4(),
        application_id: Uuid::new_v4(),
    };
    assert!(ToolHook::on_tool_event(&NoopToolHook, e).await.is_ok());
}
#[tokio::test]
async fn recorder_captures() {
    let h = RecordingToolHook::default();
    let ev = ToolHookEvent::StatusChanged {
        company_id: Uuid::new_v4(),
        application_id: Uuid::new_v4(),
        status: "active".into(),
    };
...
fn tag_is_camel_case() {
    let v: serde_json::Value = serde_json::to_value(ToolHookEvent::Patched {
        company_id: Uuid::nil(),
