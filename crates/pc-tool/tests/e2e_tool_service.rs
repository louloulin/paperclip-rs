...
async fn tool_app_crud_and_hooks() {
    ...
    let row = s.create(cid, &name, "mcp", Some("desc"), serde_json::json!({"foo":"bar"})).await.unwrap();
    ...
    let changed = s.patch(cid, row.id, ToolApplicationPatch { name: Some(format!("{name}-v2")), description: Some("updated".into()), metadata_merge: None, status: None }).await.unwrap();
    assert!(changed);
    ...
    assert!(snapshot.iter().any(|e| matches!(e, ToolHookEvent::Created { .. })));
    assert!(snapshot.iter().any(|e| matches!(e, ToolHookEvent::Patched { .. })));
    assert!(snapshot.iter().any(|e| matches!(e, ToolHookEvent::StatusChanged { .. })));
    assert!(snapshot.iter().any(|e| matches!(e, ToolHookEvent::Deleted { .. })));
    cleanup(&p, cid).await;
}
...
async fn validation_and_guards() {
    ...
    let bad = s.create(Uuid::new_v4(), "  ", "mcp", None, serde_json::json!({})).await;
    assert!(bad.is_err());
    let bad_kind = s.create(Uuid::new_v4(), "n", "", None, serde_json::json!({})).await;
    assert!(bad_kind.is_err());
    let bad_meta = s.create(Uuid::new_v4(), "n2", "mcp", None, serde_json::json!("not_object")).await;
