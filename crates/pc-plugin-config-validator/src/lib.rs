#![forbid(unsafe_code)]
//! `pc-plugin-config-validator` —— 插件实例配置 JSON Schema 验证。
//!
//! 对应 Node `server/src/services/plugin-config-validator.ts`（54 行）。
//!
//! 设计目标：1:1 复刻 `validateInstanceConfig` 的语义——
...
/// 与 Node 1:1 对齐：
/// - valid → 返回 `{ valid: true }`
/// - invalid → 返回 `{ valid: false, errors: [{ field, message }, ...] }`
/// - field 取 `instancePath`（如 `/port`），为空时退化为 `/`
/// - message 取 jsonschema 错误信息
pub fn validate_instance_config(
    ...
    match result {
        ...
        Err(err) => {
            // jsonschema 0.29 的 ValidationError 自身包含 instance_path，
            // 我们把它当作字段路径用；如果为空，退化为 `/`。
...
mod tests {
    ...
    fn r696_invalid_schema_returns_error_or_invalid() {
        ...
        let cfg = json!({});
        // 无论 schema 是 "not-a-real-type" 还是真错，都应返回 Result 不 panic
