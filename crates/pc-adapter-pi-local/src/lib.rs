...
pub struct adapter_pi_localAdapter;

impl adapter_pi_localAdapter {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl Default for adapter_pi_localAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Adapter for adapter_pi_localAdapter {
    fn descriptor(&self) -> AdapterDescriptor {
        AdapterDescriptor::builtin(ADAPTER_TYPE, "Pi")
    }
    ...
    async fn execute(
        ...
        result.provider = Some("pi_local".into());
        result.model = model;
...
mod tests {
    ...
    fn descriptor_returns_correct_type() {
        let adapter = adapter_pi_localAdapter::new();
        assert_eq!(adapter.descriptor().adapter_type, "pi_local");
    }
