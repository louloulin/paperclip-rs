//! R492 — codex adapter 真实 bridge 启动接入验证。
//!
//! 用真实 sshd fixture 验证 `start_codex_execution_bridge`（R492 接入
//! codex execute 的 bridge 启动路径）：
//! - SSH target → 真实启动完整 bridge（SSH runner + node server + worker），
//!   env 4 键可用、转发可达、teardown 清理
//! - sandbox target → `Ok(None)`（无 provider runner，保持 env-only）
//! - 本地 / 缺 target → `Ok(None)`
//! - 缺 `PAPERCLIP_API_KEY` → 报错（Node throw 语义）
//!
//! sshd / node 缺失时跳过真实部分。
...
/// 真实 sshd fixture（与 pc-acpx round492 相同模式）。
...
impl SshLabFixture {
    async fn start() -> Option<Self> {
        ...
        drop(child); // sshd 守护在后台运行，fixture drop 时 kill
...
fn execution_target_value(transport: &str, fixture: Option<&SshLabFixture>) -> serde_json::Value {
    match (transport, fixture) {
        ("ssh", Some(fixture)) => serde_json::json!({
            "transport": "ssh",
...
async fn ssh_target_starts_real_bridge_with_reachable_server() {
    ...
    // env 4 键 + server 可达（真实 node，经 SSH 启动）。
...
#[tokio::test(flavor = "multi_thread")]
async fn sandbox_target_returns_none_keeping_env_only() {
    let env = base_env();
    let target = execution_target_value("sandbox", None);
    let bridge = start_codex_execution_bridge("run-492", &env, Some(&target), None, None)
        .await
...
#[tokio::test(flavor = "multi_thread")]
async fn local_and_missing_target_return_none() {
    let env = base_env();
    let local = execution_target_value("local", None);
    let bridge = start_codex_execution_bridge("run-492", &env, Some(&local), None, None)
        .await
        .expect("local no error");
    assert!(bridge.is_none());
    let bridge = start_codex_execution_bridge("run-492", &env, None, None, None)
        .await
...
#[tokio::test(flavor = "multi_thread")]
async fn missing_host_api_key_errors() {
    let env = base_env();
    // PAPERCLIP_API_KEY 缺失 → Node throw 语义。
    let mut env_without_token = env;
