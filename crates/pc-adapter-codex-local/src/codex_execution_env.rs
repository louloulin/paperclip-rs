//! codex 执行 env 构建决策（R490）。
//!
//! 对齐 Node `codex-local/src/server/execute.ts` L806-907：`env` 构建完成后，
//! 远程 + usesBridge 时启动 paperclip bridge 并把 bridge env 合并进子进程
//! env（`Object.assign(env, paperclipBridge.env)`）。
//!
//! # 设计范围
//!
...
/// codex 执行 env 构建输入（从 adapter context 提取）。
#[derive(Debug, Clone)]
pub struct CodexExecutionEnvInput<'a> {
...
mod tests {
    ...
    ) -> CodexExecutionEnvInput<'a> {
        CodexExecutionEnvInput {
            run_id: "run-1",
            base_env,
            execution_target,
    ...
    fn local_target_returns_base_env_unchanged() {
        let base = base_env();
        let merged = build_codex_execution_env(&input(&base, Some(&json!({ "kind": "local" }))))
    ...
    fn missing_target_is_treated_as_local() {
        let base = base_env();
        let merged = build_codex_execution_env(&input(&base, None)).expect("no error");
        assert!(merged.bridge_plan.is_none());
        assert_eq!(merged.env, base);
    }
    ...
    fn remote_target_merges_four_bridge_keys() {
        let base = base_env();
        let merged = build_codex_execution_env(&input(
            &base,
    ...
    fn remote_target_without_token_errors() {
        ...
        let error = build_codex_execution_env(&input(
            &base,
            Some(&ssh_target_value("/remote/workspace")),
        ))
    ...
    fn remote_target_log_line_and_timeout() {
        let base = base_env();
        let merged = build_codex_execution_env(&CodexExecutionEnvInput {
            run_id: "run-1",
            base_env: &base,
