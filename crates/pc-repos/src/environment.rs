...
impl<'a> EnvironmentRepo<'a> {
    ...
    pub async fn list_for_company(&self, company_id: Uuid) -> RepoResult<Vec<EnvironmentRow>> {
        let sql = format!(
    ...
    pub async fn update_status(&self, id: Uuid, status: EnvironmentStatus) -> RepoResult<bool> {
        let n = sqlx::query("UPDATE environments SET status=$2, updated_at=now() WHERE id=$1")
            .bind(id)
            .bind(status.as_str())
    ...
    pub async fn merge_env_vars(&self, id: Uuid, patch: &Value) -> RepoResult<bool> {
        if !patch.is_object() {
            return Err(RepoError::Invalid(
                "env_vars patch must be an object".into(),
            ));
        }
        let n = sqlx::query(
    ...
    pub async fn update(
        ...
        if status.is_some() {
            sql.push_str(", status = $3");
        }
        if config.is_some() {
            sql.push_str(", config = $4");
        }
        let sql = format!("{} WHERE id=$1 RETURNING {ENV_COLS}", sql);
        let q = sqlx::query_as::<_, EnvironmentRow>(&sql).bind(id);
        let q = if let Some(n) = name {
            q.bind(n)
        } else {
        ...
        let q = if let Some(c) = config {
            q.bind(c)
        } else {
            q.bind(Option::<Value>::None)
        };
    ...
    pub async fn list_leases_for_company(
        ...
        let mut sql = format!("SELECT {LEASE_COLS} FROM environment_leases WHERE company_id=$1");
        if only_active {
    ...
    pub async fn acquire_lease(&self, n: &NewEnvironmentLease) -> RepoResult<EnvironmentLeaseRow> {
        let sql = format!(
    ...
    pub async fn renew_lease(&self, id: Uuid) -> RepoResult<bool> {
        let n = sqlx::query(
    ...
    pub async fn delete_custom_image_template(&self, environment_id: Uuid) -> RepoResult<u64> {
        let n =
            sqlx::query("DELETE FROM environment_custom_image_templates WHERE environment_id = $1")
                .bind(environment_id)
                .execute(self.db.pool())
                .await?
                .rows_affected();
