...
async fn link_case_document(db: &Db, company_id: Uuid, case_id: Uuid, doc_id: Uuid, key: &str) -> Uuid {
    let id = Uuid::new_v4();
    sqlx::query(
...
async fn case_documents_get_by_key() {
    ...
    let repo = CaseRepo::new(&db);
    let row = repo.get_document(cid, case_id, "design").await.expect("get").expect("present");
    assert_eq!(row.document_id, doc_id);
    assert_eq!(row.key, "design");

    // 不存在 key 返 None
    let none = repo.get_document(cid, case_id, "missing").await.expect("get");
    assert!(none.is_none());
}
...
async fn case_documents_lock_emits_event() {
    ...
    let repo = CaseRepo::new(&db);
    let n = repo.lock_document(cid, case_id, "design").await.expect("lock");
...
async fn case_documents_lock_missing_key_returns_false() {
    ...
    let case_id = insert_case(&db, cid).await;
    let repo = CaseRepo::new(&db);
    let n = repo.lock_document(cid, case_id, "ghost").await.expect("lock");
    assert!(!n);
}
...
async fn case_documents_unlock_emits_event() {
    ...
    let repo = CaseRepo::new(&db);
    let n = repo.unlock_document(cid, case_id, "design").await.expect("unlock");
    assert!(n);
...
async fn case_documents_lock_isolates_across_cases() {
    ...
    // 锁 case_a 的 document
    assert!(repo.lock_document(cid, case_a, "shared").await.expect("lock a"));
    // case_b 锁同样的 (doc_id, "shared") 不应存在
    assert!(!repo.lock_document(cid, case_b, "shared").await.expect("lock b"));
}
