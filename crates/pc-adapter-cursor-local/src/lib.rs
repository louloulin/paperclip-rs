...
pub const ADAPTER_TYPE: &str = "cursor";
...
pub struct adapter_cursor_localAdapter;

impl adapter_cursor_localAdapter {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl Default for adapter_cursor_localAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Adapter for adapter_cursor_localAdapter {
    fn descriptor(&self) -> AdapterDescriptor {
        AdapterDescriptor::builtin(ADAPTER_TYPE, "Cursor")
    }
    ...
    async fn execute(
        ...
        result.provider = Some("cursor".into());
        result.model = model;
...
mod tests {
    ...
    fn descriptor_returns_correct_type() {
        let adapter = adapter_cursor_localAdapter::new();
        assert_eq!(adapter.descriptor().adapter_type, "cursor");
    }

    #[test]
    fn default_command_falls_back_to_builtin() {
        let config = serde_json::json!({});
        assert_eq!(default_command(&config), "cursor-agent");
    }
