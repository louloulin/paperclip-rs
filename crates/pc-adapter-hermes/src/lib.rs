...
pub struct adapter_hermesAdapter;

impl adapter_hermesAdapter {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl Default for adapter_hermesAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Adapter for adapter_hermesAdapter {
    fn descriptor(&self) -> AdapterDescriptor {
        AdapterDescriptor::builtin(ADAPTER_TYPE, "Hermes")
    }
    ...
    async fn execute(
        ...
        result.provider = Some("hermes".into());
        result.model = model;
...
mod tests {
    ...
    fn descriptor_returns_correct_type() {
        let adapter = adapter_hermesAdapter::new();
        assert_eq!(adapter.descriptor().adapter_type, "hermes");
    }

    #[test]
    fn default_command_falls_back_to_builtin() {
        let config = serde_json::json!({});
        assert_eq!(default_command(&config), "hermes");
    }
