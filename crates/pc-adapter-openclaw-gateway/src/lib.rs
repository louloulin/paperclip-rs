...
pub struct adapter_openclaw_gatewayAdapter;

impl adapter_openclaw_gatewayAdapter {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl Default for adapter_openclaw_gatewayAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Adapter for adapter_openclaw_gatewayAdapter {
    fn descriptor(&self) -> AdapterDescriptor {
        AdapterDescriptor::builtin(ADAPTER_TYPE, "OpenClaw Gateway")
    }
    ...
    async fn execute(
        ...
        result.provider = Some("openclaw_gateway".into());
        result.model = model;
...
mod tests {
    ...
    fn descriptor_returns_correct_type() {
        let adapter = adapter_openclaw_gatewayAdapter::new();
        assert_eq!(adapter.descriptor().adapter_type, "openclaw_gateway");
    }

    #[test]
    fn default_command_falls_back_to_builtin() {
        let config = serde_json::json!({});
        assert_eq!(default_command(&config), "openclaw-gw");
    }
