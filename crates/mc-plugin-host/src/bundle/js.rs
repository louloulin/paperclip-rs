...
impl<'a> Scanner<'a> {
    ...
    fn identifier(&mut self, was_dot: bool) {
        let start = self.pos;
        while self.bytes.get(self.pos).is_some_and(|byte| is_ident_part(*byte)) {
            self.pos += 1;
        }
        ...
        match text {
            ...
            "import" => {
                ...
                let module_form = match self.peek(0) {
                    Some(b'.') => true,
                    Some(b'(') => false,
...
mod tests {
    ...
    fn catches_only_top_level_await() {
        ...
        assert!(!module_only("async function boot() { await fetch(\"/v1/context\"); }"));
        assert!(!module_only("boot().then(async () => { await tick(); });"));
        assert!(!module_only("const boot = async () => await fetch(\"/v1/context\");"));
        assert!(!module_only("const boot = async () => { await fetch(\"/x\"); };"));
    }

    #[test]
    fn class_method_bodies_follow_upstream_visitor() {
        // 上游 visitor 只数 FuncDecl/ArrowFunc ⇒ 类方法体里的 await 也算模块语法。
        assert!(module_only("class Panel { async load() { await fetch(\"/x\"); } }"));
    }
    ...
    fn display_messages_are_stable() {
        ...
        assert_eq!(
            JsError::UnclosedDelimiter('{').to_string(),
            "unclosed '{'"
        );
        assert_eq!(
            JsError::UnexpectedCloser(')').to_string(),
            "unexpected ')'"
        );
    }
}
