...
impl TokenError {
    ...
    pub const fn code(&self) -> &'static str {
        match self {
            ...
            Self::InvalidInstallToken
            | Self::InvalidCallbackToken
            | Self::CallbackTokenUnavailable => "plugin_token_invalid",
        }
    }
}
...
mod tests {
    ...
    fn callback_token_survives_a_second_call() {
        ...
        let token = tokens
            ...
            .expect("issue");
        let first = tokens
            .resolve_at(&token, 1_700_000_060)
            .expect("first call");
        let second = tokens
    ...
    fn error_codes_and_messages_match_upstream() {
        assert_eq!(
            TokenError::InvalidInstallToken.to_string(),
            "invalid plugin token"
        );
        assert_eq!(
            TokenError::InvalidCallbackToken.to_string(),
        ...
        assert_eq!(
            TokenError::InvalidInstallToken.code(),
