#![forbid(unsafe_code)]
//! `pc-database-backup-health` —— 数据库备份健康检查。
//!
//! 对应 Node `server/src/services/database-backup-health.ts`（153 行）。
//!
//! 设计目标：1:1 复刻
//! - 检查 `backupDir` 中的 `.sql.gz` 文件 → latest mtime / age / size
//! - 检查 alert files（failure markers）→ latest failure message
//! - 计算 warnings：`missing` / `stale` / `last_failure` / `check_failed`
//!
//! 与 Node 的差异：
//! - 文件系统操作通过 [`FsOps`] trait 注入（测试可注入 fake fs）
//! - `now` 通过 `InspectDatabaseBackupHealthOptions` 注入
