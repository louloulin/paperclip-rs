//! R428 — gemini-local 端到端验证「resume 失败 → 真实重跑」主回路。
//!
//! 复刻 Node `packages/adapters/gemini-local/src/server/execute.ts` 中
//! `initial = runAttempt(sessionId); isGeminiSessionUnrecoverableError → runAttempt(null)` 回路。
...
fn write_mock(dir: &std::path::Path, body: &str) -> PathBuf {
    let script = dir.join("gemini-mock");
...
async fn unknown_session_triggers_real_retry_without_resume() {
    let tmp = TempDir::new("gemini-retry");
    let log = tmp.path.join("calls.log");
    let body = format!(
        "#!/bin/sh\necho \"$@\" >> '{log}'\nN=$(wc -l < '{log}' | tr -d ' ')\nif [ \"$N\" = \"1\" ]; then\n  echo 'unknown session cannot resume' >&2\n  exit 1\nelse\n  printf '%s\\n' '{{\"type\":\"result\",\"status\":\"success\",\"is_error\":false,\"session_id\":\"sess-fresh\",\"result\":\"Recovered\"}}'\n  exit 0\nfi\n",
        log = log.display()
    );
    ...
    let result = GeminiLocalAdapter::new()
        .execute(
...
async fn unrelated_failure_does_not_retry() {
    let tmp = TempDir::new("gemini-rate");
    let log = tmp.path.join("calls.log");
    let body = format!(
