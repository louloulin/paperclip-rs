//! R393 — Integration tests for `pc-adapter-gemini-local::skills`.
//!
//! Mirrors Node `packages/adapters/gemini-local/src/server/skills.ts`:
//! - `listGeminiSkills` (L42-44)
//! - `syncGeminiSkills` (L46-72) — the **first adapter with side-effecting sync**
//! - `resolveGeminiSkillsHome` (L17-24)
//! - `resolveGeminiDesiredSkillNames` (L74-77)
//! - `buildGeminiSkillSnapshot` (L26-40)
//!
//! Unit tests inside `skills::tests` cover each function in isolation;
//! this integration suite verifies the end-to-end sync flow:
//! create + repair + remove symlinks.

use pc_acpx::AdapterSkillContext;
use pc_adapter_gemini_local::skills::{
    build_gemini_skill_snapshot, list_gemini_skills, resolve_gemini_desired_skill_names,
    resolve_gemini_skills_home, resolve_gemini_skills_home_with, sync_gemini_skills,
    GEMINI_SKILLS_HOME_SUFFIX,
};
use serde_json::json;
...
fn unique_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "pc-adapter-gemini-local-r393-{label}-{nanos}-{}",
        std::process::id()
    ))
}
...
fn make_ctx(config: serde_json::Value) -> AdapterSkillContext {
    AdapterSkillContext::new("agent-r393", "company-r393", "gemini_local", config)
}
...
#[test]
fn gemini_skills_home_suffix_is_dot_gemini_skills() {
    assert_eq!(GEMINI_SKILLS_HOME_SUFFIX, ".gemini/skills");
}

// ---------------------------------------------------------------------------
// resolve_gemini_skills_home — end-to-end
// ---------------------------------------------------------------------------

#[test]
fn skills_home_prefers_env_home_when_set() {
    let config = json!({ "env": { "HOME": "/srv/gemini" } });
    let resolved = resolve_gemini_skills_home(&config).unwrap();
    assert_eq!(resolved, PathBuf::from("/srv/gemini/.gemini/skills"));
}

#[test]
fn skills_home_with_fallback_uses_default() {
    let resolved = resolve_gemini_skills_home_with(&json!({}), "/home/x");
    assert_eq!(resolved, PathBuf::from("/home/x/.gemini/skills"));
}

#[test]
fn skills_home_with_fallback_honours_env_override() {
    let config = json!({ "env": { "HOME": "/explicit" } });
    let resolved = resolve_gemini_skills_home_with(&config, "/default");
    assert_eq!(resolved, PathBuf::from("/explicit/.gemini/skills"));
}

// ---------------------------------------------------------------------------
// list_gemini_skills — end-to-end
// ---------------------------------------------------------------------------

#[tokio::test]
async fn list_gemini_skills_returns_supported_snapshot() {
...
#[tokio::test]
async fn list_gemini_skills_surfaces_warning_when_no_skills_home() {
...
// ---------------------------------------------------------------------------
// sync_gemini_skills — end-to-end (the key new behaviour)
// ---------------------------------------------------------------------------

#[tokio::test]
#[cfg(unix)]
async fn sync_gemini_skills_end_to_end_full_lifecycle() {
    ...
    // 1. Sync with both desired → a & b symlinks created, external
    //    remains untouched.
    let desired = vec![
        "paperclipai/paperclip/a".to_string(),
        "paperclipai/paperclip/b".to_string(),
    ];
    let snap1 = sync_gemini_skills(&ctx, &desired, &module_dir, &skills_home).await;
    assert_eq!(snap1.adapter_type, "gemini_local");
    ...
    assert!(tokio::fs::symlink_metadata(skills_home.join("b"))
        ...
        .file_type()
        .is_symlink());
    // External symlink remains.
    let meta = tokio::fs::symlink_metadata(skills_home.join("external-symlink"))
        .await
        .unwrap();
    assert!(meta.file_type().is_symlink());

    // 2. Sync with only "a" desired → "b" symlink (which was
    //    Paperclip-managed) should be removed.
    let desired_a_only = vec!["paperclipai/paperclip/a".to_string()];
    let _ = sync_gemini_skills(&ctx, &desired_a_only, &module_dir, &skills_home).await;
    assert!(tokio::fs::symlink_metadata(skills_home.join("a"))
        .await
        .is_ok());
    assert!(tokio::fs::symlink_metadata(skills_home.join("b"))
    ...
    // 3. Sync with no desired → "a" symlink also removed.
...
#[tokio::test]
async fn sync_gemini_skills_accepts_empty_desired_skills() {
    ...
    let snap = sync_gemini_skills(&ctx, &[], &module_dir, &skills_home).await;
    assert_eq!(snap.adapter_type, "gemini_local");
    // Snapshots always list available entries from paperclipRuntimeSkills;
    // sync() does not mutate that surface, only the installed symlinks.
...
// ---------------------------------------------------------------------------
// resolve_gemini_desired_skill_names — end-to-end
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// build_gemini_skill_snapshot — direct call parity
// ---------------------------------------------------------------------------
...
async fn build_snapshot_matches_list_call() {
    ...
    let ctx = make_ctx(json!({ "env": { "HOME": parent.to_string_lossy() } }));
    let via_list = list_gemini_skills(&ctx, &module_dir, Some(&skills_dir)).await;
    let via_build = build_gemini_skill_snapshot(&ctx, &module_dir, &skills_dir).await;
