...
fn accumulate(usage: &mut UsageSummary, raw: Option<&Value>) {
    ...
    let source = object
        ...
        .unwrap_or(object);
    usage.input_tokens +=
        number_field(source, &["input_tokens", "inputTokens", "promptTokenCount"]);
    if let Some(cached) = usage.cached_input_tokens.as_mut() {
        *cached += number_field(
            source,
            &[
                "cached_input_tokens",
    ...
    usage.output_tokens += number_field(
        source,
...
pub fn parse_gemini_stream_json(stdout: &str) -> ParsedGeminiStreamJson {
    ...
    for raw_line in stdout.lines() {
        ...
        match event_type.as_str() {
                                ...
                                let choices = part
                                    ...
                                    .map(|items| {
                                        items
                                            ...
                                            .map(|item| GeminiQuestionChoice {
                                                key: string_field(item, "key"),
                                                label: string_field(item, "label"),
                                                description: (!string_field(item, "description")
                                                    .is_empty())
                                                .then(|| string_field(item, "description")),
                                            })
                                            .collect()
                                    })
                                    .unwrap_or_default();
                                parsed.question = Some(GeminiQuestion {
                                    prompt: string_field(part, "prompt"),
                                    choices,
                                });
                                break;
                            }
            ...
            "message" if string_field(&event, "role").to_ascii_lowercase() == "assistant" => {
                messages.extend(message_text(event.get("content")))
            }
            "result" => {
                accumulate(
                    &mut parsed.usage,
                    event
                        ...
                        .or_else(|| event.get("stats")),
                );
                let result_text = ["result", "text", "response"]
                    .iter()
                ...
                parsed.cost_usd = ["total_cost_usd", "cost_usd", "cost"]
                    .iter()
            ...
            "system" if string_field(&event, "subtype").to_ascii_lowercase() == "error" => {
                let text = error_text(
                    event
                        .get("error")
                        .or_else(|| event.get("message"))
            ...
            "step_finish" => {
                accumulate(
                    &mut parsed.usage,
                    event.get("usage").or_else(|| event.get("usageMetadata")),
                );
            }
            _ if event.get("usage").is_some() || event.get("usageMetadata").is_some() => {
                accumulate(
                    &mut parsed.usage,
                    event.get("usage").or_else(|| event.get("usageMetadata")),
                )
...
{"type":"result","session_id":"s1","stats":{"input_tokens":10,"cached":3,"output_tokens":4},"result":"done"}"#,
    ...
    fn question和错误可解析() {
        let parsed = parse_gemini_stream_json(
            r#"{"type":"assistant","message":{"content":[{"type":"question","prompt":"继续?","choices":[{"key":"y","label":"是","description":"继续执行"}]}]}}
...
pub fn describe_gemini_failure(parsed: &Value) -> Option<String> {
    ...
    if status.is_empty() && detail.is_empty() {
        return None;
    }
...
pub fn detect_gemini_auth_required(parsed: Option<&Value>, stdout: &str, stderr: &str) -> bool {
    let parsed_text = parsed
        ...
        .map(|object| error_text(object.get("error").or_else(|| object.get("message"))))
        .unwrap_or_default();
...
pub fn is_gemini_turn_limit_result(parsed: Option<&Value>, exit_code: Option<i32>) -> bool {
    ...
    .any(|value| {
        matches!(
            value.as_str(),
            "turn_limit" | "max_turns" | "max_turns_exhausted" | "turn_limit_exhausted"
        )
    })
}
