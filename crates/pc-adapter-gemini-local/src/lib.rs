...
pub struct adapter_gemini_localAdapter;

impl adapter_gemini_localAdapter {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl Default for adapter_gemini_localAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Adapter for adapter_gemini_localAdapter {
    ...
    async fn execute(
        ...
        result.provider = Some("gemini_local".into());
        result.model = model;
...
mod tests {
    ...
    fn descriptor_returns_correct_type() {
        let adapter = adapter_gemini_localAdapter::new();
        assert_eq!(adapter.descriptor().adapter_type, "gemini_local");
    }
