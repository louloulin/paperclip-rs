...
fn haystack(
    ...
    text_values(parsed, stdout, stderr, error_message)
        .join("\n")
        .to_ascii_lowercase()
}
...
pub fn describe_claude_failure(parsed: &Value) -> Option<String> {
    ...
    if subtype.is_empty() && detail.is_none() {
        return None;
    }
...
pub fn is_claude_model_not_found_error(
    parsed: Option<&Value>,
    stdout: &str,
    stderr: &str,
    error_message: Option<&str>,
...
pub fn is_claude_refusal_result(parsed: Option<&Value>) -> bool {
    ...
    .filter_map(|key| object.get(*key).and_then(Value::as_str))
    .map(|value| value.trim().to_ascii_lowercase())
    .any(|value| value == "model_refusal" || value == "refusal")
}

pub fn is_claude_poisoned_previous_message_id_error(parsed: &Value) -> bool {
    haystack(Some(parsed), "", "", None).contains("diagnostics.previous_message_id")
        && haystack(Some(parsed), "", "", None).contains("starts with `msg_")
}

pub fn is_claude_transient_upstream_error(
    parsed: Option<&Value>,
    stdout: &str,
    stderr: &str,
    error_message: Option<&str>,
) -> bool {
    if is_claude_max_turns_result(parsed)
        || is_claude_unknown_session_error(parsed)
        || parsed.is_some_and(|value| is_claude_poisoned_previous_message_id_error(value))
        || parsed
            .is_some_and(|value| crate::claude_stream_json::is_claude_image_processing_error(value))
        || is_claude_login_required(parsed, stdout, stderr)
        || is_claude_provider_quota_error(parsed, stdout, stderr, error_message)
    {
        return false;
    }
    let text = haystack(parsed, stdout, stderr, error_message);
...
pub fn is_claude_provider_quota_error(
    parsed: Option<&Value>,
    stdout: &str,
    stderr: &str,
    error_message: Option<&str>,
...
pub fn is_claude_unknown_session_error(parsed: Option<&Value>) -> bool {
    ...
    .any(|needle| text.contains(needle))
        && (text.contains("not found")
            ...
            || text.contains("requires a valid session")
...
pub fn extract_claude_retry_not_before(text: &str, now: SystemTime) -> Option<SystemTime> {
    ...
    let clock = if first.ends_with('a')
        || first.ends_with('p')
        || first.ends_with("am")
        || first.ends_with("pm")
    {
    ...
    if is_pm {
        target = current_day_start + (hour + 12) * 3600 + minute * 60;
    }
    if target <= current_day_start + current_seconds {
        target += 86_400;
    }
...
fn parse_clock(text: &str) -> Option<(u64, u64, bool)> {
    ...
    (1..=12)
        .contains(&hour)
        .then_some((hour % 12, minute, is_pm))
        .filter(|(_, minute, _)| *minute < 60)
}
...
pub fn claude_model_usage_totals(model_usage: &Value) -> Option<UsageSummary> {
    ...
    Some(UsageSummary {
        input_tokens,
        output_tokens,
        cached_input_tokens: if saw_cached {
