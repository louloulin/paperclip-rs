//! Claude adapter config schema（控制台表单字段定义）。
//!
//! 严格对齐 Node `packages/adapters/claude-local/src/server/config-schema.ts`：
...
pub fn get_config_schema() -> AdapterConfigSchema {
    AdapterConfigSchema::new(vec![
        ...
        ConfigFieldSchema {
            ...
            default: None,
            hint: Some(
                "Optional override for the Claude ACP server command. Defaults to the package-local claude-agent-acp binary."
                    .to_string(),
            ),
            required: None,
            group: None,
