...
pub fn claude_model_usage_totals(model_usage: Option<&Value>) -> Option<UsageSummary> {
    ...
    for value in models.values() {
        ...
        saw_entry = true;
        totals.input_tokens += number_field(entry, &["inputTokens"])
            + number_field(entry, &["cacheCreationInputTokens"]);
...
pub fn parse_claude_stream_json(stdout: &str) -> ParsedClaudeStreamJson {
    ...
    for raw_line in stdout.lines() {
        ...
        match string_field(&event, "type").as_deref() {
            Some("system") if string_field(&event, "subtype").as_deref() == Some("init") => {
                if let Some(session) = string_field(&event, "session_id") {
                    parsed.session_id = Some(session);
                }
                if let Some(model) = string_field(&event, "model") {
            ...
            Some("result") => {
                ...
                if event
                    ...
                    .unwrap_or(false)
                {
                    parsed.error_message = string_field(&event, "result").or_else(|| {
                        string_field(&event, "subtype").map(|s| format!("error_subtype={s}"))
                    });
                }
    ...
    parsed.usage = claude_model_usage_totals(object.get("modelUsage")).or_else(|| {
        object
            ...
            .map(|usage| UsageSummary {
                ...
                cached_input_tokens: Some(number_field(
                    usage,
    ...
    parsed.summary = string_field(object, "result")
        .unwrap_or_else(|| assistant_texts.join("\n\n"))
        .trim()
        .to_owned();
...
fn messages_from_result(parsed: &Value) -> Vec<String> {
    ...
    if let Some(text) = string_field(object, "result") {
        messages.push(text);
    }
    if let Some(Value::Array(errors)) = object.get("errors") {
        for error in errors {
            match error {
                ...
                Value::Object(error) => {
                    if let Some(text) = string_field(error, "message")
                        .or_else(|| string_field(error, "error"))
                        .or_else(|| string_field(error, "code"))
                    {
                        messages.push(text);
                    } else if let Ok(text) = serde_json::to_string(error) {
                        messages.push(text);
                    }
                }
                _ => {}
...
pub fn detect_claude_login_required(parsed: Option<&Value>, stdout: &str, stderr: &str) -> bool {
    ...
    if let Some(parsed) = parsed {
        for message in messages_from_result(parsed) {
...
pub fn is_claude_unknown_session_error(parsed: &Value) -> bool {
    messages_from_result(parsed).iter().any(|message| {
        ...
        lower.contains("no conversation found with session id")
            ...
            || lower.contains("not a valid uuid")
            || lower.contains("--resume requires a valid session")
            || lower.contains("does not match any session title")
    })
}
...
{"type":"result","session_id":"s1","result":"done","total_cost_usd":1.2,"usage":{"input_tokens":10,"output_tokens":4,"cache_read_input_tokens":2}}"#,
    ...
    fn model_usage优先且缓存创建计入输入() {
        let parsed = parse_claude_stream_json(
            r#"{"type":"result","result":"ok","modelUsage":{"opus":{"inputTokens":10,"cacheCreationInputTokens":3,"cacheReadInputTokens":20,"outputTokens":4}}}"#,
        );
    ...
    fn 登录和未知会话识别() {
        ...
        assert!(is_claude_unknown_session_error(
            &serde_json::json!({"result":"No conversation found with session id x"})
        ));
        assert!(is_claude_image_processing_error(
            &serde_json::json!({"result":"400 Could not process image"})
        ));
    }
}
