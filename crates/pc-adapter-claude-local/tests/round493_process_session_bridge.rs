//! R493 — claude adapter 真实 process session bridge 启动接入验证。
//!
//! 用本地进程 runner（`LocalProcessBridgeRunner`）+ 临时目录模拟 sandbox
//! target，真实 node 远端脚本 + 真实 TCP server + 真实 proxy 脚本跑通
//! `start_claude_process_session_bridge`（R493 接入 claude execute 的
...
use pc_adapter_claude_local::claude_remote_workspace::start_claude_process_session_bridge;
...
impl LocalSandbox {
    fn new(child_source: &str) -> Self {
        let root_dir = std::env::temp_dir().join(format!(
            "paperclip-claude-process-session-{}",
            uuid::Uuid::new_v4()
        ));
