...
//! - **上游**：`internal/integrations/**` 的 `AuditDrop` 面。
//! - **语义**：**非内容**丢弃审计的写入与查询：`drop_reason` / `event_type` / `channel_event_id` / `channel_message_id`；`installation_id` **可空**（`124` 去掉了 `ON DELETE SET NULL`，改成应用层可留空）。
//! - **硬约束**：**不得**把消息正文写进审计表（非内容口径是这张表存在的理由）。
