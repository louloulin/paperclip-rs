#![forbid(unsafe_code)]
//! `pc-responsible-user-denial` —— responsible-user denial code 规范化。
//!
//! 对应 Node `server/src/services/responsible-user-denial-run-outcomes.ts`（75 行）。
//!
//! 设计目标：1:1 复刻
//! - `ResponsibleUserDenialCode` 枚举
//! - `normalizeResponsibleUserDenialCode(value)` —— 仅当 value 是合法 code 时返回
//!   Some(code)，其它一律 `None`
//!
//! DB 写操作（`recordResponsibleUserDenialOnActiveRun`）由上层接入 pc-repos。

/// Responsible-user denial code 枚举 —— 与 Node `ResponsibleUserDenialCode` 1:1 对齐。
///
/// 注：完整集合在 `@paperclipai/shared`，这里列出本 crate 关心的子集。
/// 如果之后新增类别，需要同步扩展 `is_valid_code` 的判断。
...
/// Overload —— 接受 `Option<&str>` 或 `&serde_json::Value`。
pub fn normalize_responsible_user_denial_code_value(value: &serde_json::Value) -> Option<ResponsibleUserDenialCode> {
