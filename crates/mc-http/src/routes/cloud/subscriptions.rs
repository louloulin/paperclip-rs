//! `/api/cloud-subscriptions/*` 7 条（**写者 M9-2** / `docs/62` §4.1 的第 3 行）。
//!
...
//! 🔴 **不得**在这里注册 501 占位：这 7 条是**上游键**，占位会让 ⑦ 把它们算成
//! `implemented_placeholder`（`owners.M9` 假清零），而 ⑨ 会从 `unmounted` 变 `mismatch`。
//!
//! 上游：`internal/handler/cloud_billing.go` 的 `L54–L355`。读 2 条 member 可读、写 5 条 owner|admin；
//! flag 关 ⇒ 403 `workspace_subscriptions_disabled`（`docs/62` §2.5 的 B 行）。
//! 出站路径表见 `mc-cloud/src/subscriptions.rs` 的模块头；幂等键两档上限（255 / 200）在
//! `mc_core::cloud`。
//!
...
/// subscriptions 切片（M9-2 在这里挂 7 条 workspace 级代理）。
