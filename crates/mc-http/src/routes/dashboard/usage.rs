//! `/api/dashboard/usage/{daily,by-agent}` 2 条（**写者 M9-4**）。
//!
...
//!
//! 上游：`internal/handler/dashboard.go` 的 `GetDashboardUsageDaily` / `GetDashboardUsageByAgent`。
//! 行形状在 `mc_core::dashboard`（anchor 定形）；`by-agent` **必须**在服务端折叠私有 agent
//! 到 `__restricted_agents__` 哨兵。
//!
