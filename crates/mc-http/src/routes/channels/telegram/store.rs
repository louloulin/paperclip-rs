...
//!
//! `PgInstallStore::get_in_workspace` / `revoke` / `PgBindingStore::insert_token` 逐字对应上游同名
//! 语句，但多一条 `channel_type = 'telegram'` 的收窄（泛化表跨渠道）。
//!
//! # 与上游 `Update`（`GetChannelInstallationByAppID`）的一处措辞差异
//!
//! 上游 `persistInstall` 的"回收死主"用的是 `ReclaimDeadChannelInstallationByAppID`：把
//! **已撤销**或**所有者已被删除**的旧主占着的 `(channel_type, app_id)` 槽让出来。
//! 本文件逐字照它的判据（`status = 'active'` ∧ 不是本次 (workspace, agent) ∧ agent 不存在），
//! 只是写成本仓的 `UPDATE … WHERE … NOT EXISTS` 形态（与 M7-4 的 slack 实现同款）。
//!
//! # 凭据纪律
//!
//! 本文件只搬运**已经封好的** `config`（`secretbox` 密文）；它**不解密**、不打印、不拼错误文案。
//! 绑定那一段多一条**渠道收窄**（`channel_type = 'telegram'`）：令牌表跨 adapter 共享，
//! 别的 adapter 的令牌**不该**能从 Telegram 兑换 —— 上游是"先 consume 再校验、失败靠回滚"，
//! 本仓把这条写进 `WHERE`（**等价**：认不出 ⇒ 令牌**没被消费**）。

use chrono::{DateTime, Utc};
use mc_channel::telegram::binding::{BindingStore, NewBindingToken, RedeemOutcome};
use mc_channel::telegram::install::{InstallRecord, InstallStore, PersistInstall, PersistOutcome};
use mc_core::id::Id;
use sqlx::FromRow;
use uuid::Uuid;
...
/// 泛化安装表的 Telegram 实现（上游 `installQueries` 的六条语句 + `persistInstall` 的事务）。
#[derive(Debug, Clone)]
...
impl InstallStore for PgInstallStore {
    async fn list_by_workspace(&self, workspace_id: Id) -> Result<Vec<InstallRecord>, String> {
        let sql = format!(
            "SELECT {INSTALL_COLUMNS} FROM channel_installation \
             WHERE workspace_id = $1 AND channel_type = 'telegram' \
    ...
    async fn get_in_workspace(
        ...
        let sql = format!(
            "SELECT {INSTALL_COLUMNS} FROM channel_installation \
             WHERE id = $1 AND workspace_id = $2 AND channel_type = 'telegram'"
        );
    ...
    async fn persist(&self, params: &PersistInstall) -> Result<PersistOutcome, String> {
        ...
        // 2) 按 (workspace, agent, channel_type) upsert：**一个 agent 一个 bot**（上游逐字：
        //    ONE Telegram bot per agent）。修 bug 的隐蔽点：唯一索引是
        //    `(channel_type, app_id)`，所以"贴了别人的 bot"会在这里撞。
        let sql = format!(
...
impl PgInstallStore {
    /// 唯一冲突之后**谁占着**这个 `(telegram, app_id)` 槽（上游 `liveOwnerConflictErr`）。
    ///
...
/// 泛化绑定表的 Telegram 实现（上游 `binding.go` 的三条语句 + 事务）。
#[derive(Debug, Clone)]
...
impl BindingStore for PgBindingStore {
    ...
    ///
    /// 三段的顺序与回滚语义逐字照上游：非成员 / 已属于别人**不烧掉**令牌（回滚消费）。
    /// 消费那一步额外按 `channel_type = 'telegram'` 收窄（见模块文档）。
    async fn redeem_and_bind(
        ...
        let consumed: Option<BindingTokenRow> = sqlx::query_as(
            "UPDATE channel_binding_token SET consumed_at = now() \
             WHERE token_hash = $1 AND channel_type = 'telegram' \
        ...
        Ok(RedeemOutcome::Bound(
            mc_channel::telegram::binding::RedeemedBinding {
