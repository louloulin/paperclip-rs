//! 插件包用例的 zip 夹具（M6-5）。
//!
//! 与 `tests/skills/zipfixture.rs` 是**两份**：每个 `tests/<dir>/main.rs` 是独立 crate，
//! 跨目录共享要走 `#[path]` 或公共 dev-dep，都不如复制这 100 行便宜。
//!
//! 只用 **store**（method 0）方法、不压缩：`zip` crate 会校验 CRC，所以夹具必须给出正确的
//! CRC32 —— 手写一个约 10 行的位运算实现在夹具里就是全部代价；换成真压缩器只会让「夹具是不是
//! 被损坏了」这件事更难判断。

/// 用 store 方法打一个 zip（`mc-plugin-host` 的 `parse_bundle` 只按条目读）。
...
/// 夹具自身的自检（真跑一次解析，防止「CRC 写错 ⇒ 全部上传用例一起假红」）。
#[test]
fn zip_store_fixture_is_readable() {
    let archive = zip_store(&[
        ("multica.plugin.json", "{\"manifest_version\":1}"),
