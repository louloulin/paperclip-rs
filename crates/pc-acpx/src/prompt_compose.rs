...
pub struct NormalizedPaperclipWake {
    ...
    pub active_tree_hold: Option<Value>,
    ...
    pub plan_review_context: Option<Value>,
    pub task_watchdog: Option<Value>,
    pub liveness_continuation: Option<Value>,
    pub annotation_deltas: Vec<Value>,
    pub continuation_summary: Option<Value>,
    pub unresolved_blocker_summaries: Vec<Value>,
}
...
fn normalize_execution_workspace(v: Option<&Value>) -> Option<PaperclipWakeExecutionWorkspace> {
    let obj = opt_object(v)?;
    Some(PaperclipWakeExecutionWorkspace {
        branch_name: opt_string(obj.get("branchName")),
        workspace_id: opt_string(obj.get("workspaceId")),
    })
}
...
pub fn normalize_paperclip_wake_payload(value: Option<&Value>) -> Option<NormalizedPaperclipWake> {
    ...
    let active_tree_hold = obj.get("activeTreeHold").cloned();
    let plan_review_context = obj.get("planReviewContext").cloned();
    let task_watchdog = obj.get("taskWatchdog").cloned();
    let liveness_continuation = obj.get("livenessContinuation").cloned();
    let continuation_summary = obj.get("continuationSummary").cloned();

    let annotation_deltas: Vec<Value> = obj
        .get("annotationDeltas")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().cloned().collect())
        .unwrap_or_default();

    let unresolved_blocker_summaries: Vec<Value> = obj
...
fn markdown_inline_code(value: &str) -> String {
    let longest = value
        ...
        .fold((0usize, 0usize), |(max, cur), c| {
            if c == '`' {
                let next = cur + 1;
                if next > max { (next, next) } else { (max, next) }
            } else {
                (max, 0)
            }
        })
...
pub fn render_paperclip_wake_prompt(
    ...
    let recovery_scoped = recovery.is_some()
        || normalized.reason.as_deref() == Some("source_scoped_recovery_action");
    ...
    wake_summary_lines.push(format!(
        ...
        if normalized.fallback_fetch_needed { "yes" } else { "no" }
    ));
    if recovery_scoped {
        wake_summary_lines.push(format!(
            "- recovery cause: {}",
            recovery.and_then(|r| r.cause.as_deref()).unwrap_or("unknown")
        ));
        wake_summary_lines.push(format!(
            "- failure summary: {}",
            recovery
        ...
        wake_summary_lines.push(format!(
            "- next action: {}",
            recovery.and_then(|r| r.next_action.as_deref()).unwrap_or("unknown")
        ));
    ...
    if let Some(description) = &issue_description {
        if !options.suppress_issue_description && !resume_omits_issue_description {
            ...
            if normalized
                ...
                .unwrap_or(false)
            {
                lines.push("[issue description truncated; fetch the issue for the full brief]".to_string());
            }
    ...
    if let Some(issue) = &normalized.issue {
        if issue.work_mode.as_deref() == Some("planning") && normalized.task_watchdog.is_none() {
            ...
            let mut directive = "Make the plan only. Do not write code or perform implementation work.";
    ...
    if normalized.dependency_blocked_interaction {
        ...
        if !normalized.unresolved_blocker_summaries.is_empty() {
            let blockers: Vec<String> = normalized
                ...
                .filter_map(|v| {
                    let obj = v.as_object()?;
                    let id = obj.get("id").and_then(|x| x.as_str()).map(|s| s.to_string());
                    let identifier = obj
                        .get("identifier")
                        .and_then(|x| x.as_str())
                        .map(|s| s.to_string());
                    let title = obj.get("title").and_then(|x| x.as_str()).map(|s| s.to_string());
                    let status = obj
                        .get("status")
                        .and_then(|x| x.as_str())
                        .map(|s| s.to_string());
                    let key = identifier.or(id).unwrap_or_else(|| "unknown".to_string());
                    let mut s = key;
                    if let Some(t) = title {
                        s.push_str(&format!(" {}", t));
                    }
                    if let Some(s2) = status {
    ...
    if normalized.tree_hold_interaction {
        ...
        if let Some(hold) = &normalized.active_tree_hold {
            let hold_id = hold.get("holdId").and_then(|v| v.as_str()).unwrap_or("unknown");
            let root = hold.get("rootIssueId").and_then(|v| v.as_str());
            let mode = hold.get("mode").and_then(|v| v.as_str());
    ...
    if let Some(stage) = &normalized.execution_stage {
        ...
        if !stage.allowed_actions.is_empty() {
            lines.push(format!("- allowed actions: {}", stage.allowed_actions.join(", ")));
        }
        lines.push(format!(
    ...
    if normalized.plan_review_context.is_some() {
        lines.push("- plan review context: present (R382+ will render full threads)".to_string());
    }
    if normalized.task_watchdog.is_some() {
        lines.push("- task watchdog: present (R382+ will render full mandate)".to_string());
    }
    if normalized.liveness_continuation.is_some() {
        lines.push("- liveness continuation: present (R382+ will render full block)".to_string());
    }
    if !normalized.annotation_deltas.is_empty() {
        lines.push(format!(
            "- annotation deltas: {} (R382+ will render full content)",
            normalized.annotation_deltas.len()
        ));
    }
    if normalized.continuation_summary.is_some() {
        lines.push("- continuation summary: present (R382+ will render full block)".to_string());
    }
...
// ----------------------------------------------------------------------------
    ...
    fn normalize_parses_recovery_block_with_cause_and_attempts() {
        ...
        assert_eq!(rec.failure_summary.as_deref(), Some("subprocess died at step 5"));
    ...
    fn render_recovery_wake_includes_cause_specific_instruction() {
        ...
        assert!(rendered.contains(
            "Your previous run on this issue was lost"
        ));
    }
    ...
    fn render_planning_issue_emits_directive() {
        ...
        let rendered = render_paperclip_wake_prompt(
            Some(&payload),
            &RenderWakePromptOptions::default(),
        );
        eprintln!("RENDERED_PLACEHOLDER_BEGIN\n{}\nRENDERED_PLACEHOLDER_END", rendered);
    ...
    fn render_planning_with_accepted_continuation_directive() {
        ...
        assert!(rendered.contains(
            "- planning directive: Create child issues from the approved plan only."
        ));
