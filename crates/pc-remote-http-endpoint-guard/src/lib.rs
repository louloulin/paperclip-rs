#![forbid(unsafe_code)]
//! pc-remote-http-endpoint-guard -- Remote HTTP endpoint URL validation + private/reserved IP guard
//!
//! Corresponds to Node `server/src/services/remote-http-endpoint-guard.ts` (161 lines).
//!
//! Design goals: 1:1 reproduction
//! - parse_remote_http_endpoint -- protocol check (http/https) + non-empty
//! - assert_public_remote_http_endpoint -- check hostname resolves to non-private IP
//! - Full IPv4/IPv6 private/reserved detection (incl. IPv4-mapped IPv6)
//! - DNS lookup via trait injection (fake for tests; node:dns in production)
