#![forbid(unsafe_code)]
//! `pc-inbox-dismissals` —— inbox dismissal 纯逻辑 helper。
//!
//! 对应 Node `server/src/services/inbox-dismissals.ts`（75 行）。
//!
//! 设计目标：1:1 复刻
//! - `InboxDismissalKind` 枚举（dismiss / snooze）
//! - `compute_snoozed_until(kind, snoozed_until)` —— 只有 kind == "snooze" 才
//!   保留 snoozedUntil，其它情况一律为 None
//!
//! DB 部分（`inboxDismissalService(db)`）由上层接入 pc-repos。
