...
//!
//! 仅 port 纯函数部分；DB-touching 的 `instanceSettingsService` 和
//! async env-touching 的 `resolve_worktree_run_execution_activation_state`
//! 不在本模块，落在 `instance_settings::service`。
//!
...
/// Singleton key for the `instance_settings` row（对齐 Node `DEFAULT_SINGLETON_KEY`）。
pub const DEFAULT_SINGLETON_KEY: &str = "default";
...
/// Public fallback clock used by the service layer when no `now` closure is
/// injected. Kept separate from the private `now_iso` so the `service` module
/// imports this without reaching into module-private helpers.
pub(crate) fn now_iso_for_test() -> String {
    now_iso()
}
