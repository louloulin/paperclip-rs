...
impl Config {
    pub fn from_env() -> Result<Self, ConfigError> { Self::from_env_with_dotenv(None) }

    pub fn from_env_with_dotenv(dotenv_path: Option<&Path>) -> Result<Self, ConfigError> {
        if let Some(p) = dotenv_path { dotenvy::from_path(p).ok(); } else { dotenvy::dotenv().ok(); }
        Self::build()
    }
    ...
    pub fn build_with<F: Fn(&str) -> Option<String>>(lookup: F) -> Result<Self, ConfigError> {
        let lookup = &lookup;
        let mode = match lookup("PAPERCLIP_MODE").unwrap_or_else(|| "development".into()).to_lowercase().as_str() {
        ...
        let database = DatabaseConfig {
            url: lookup("PAPERCLIP_DATABASE_URL").ok_or(ConfigError::MissingEnv("PAPERCLIP_DATABASE_URL"))?,
        ...
        let auth = AuthConfig {
            session_cookie_name: lookup("PAPERCLIP_SESSION_COOKIE").unwrap_or_else(|| "paperclip.session".into()),
            session_ttl_secs: parse_or(lookup, "PAPERCLIP_SESSION_TTL_SECS", 60 * 60 * 24 * 30)?,
            api_key_header: lookup("PAPERCLIP_API_KEY_HEADER").unwrap_or_else(|| "x-paperclip-agent-key".into()),
            csrf_header: lookup("PAPERCLIP_CSRF_HEADER").unwrap_or_else(|| "x-paperclip-csrf".into()),
        };

        let storage = StorageConfig {
            kind: match lookup("PAPERCLIP_STORAGE").unwrap_or_else(|| "local-disk".into()).as_str() {
                "s3" => StorageKind::S3,
        ...
        let secrets = SecretsConfig {
            kind: match lookup("PAPERCLIP_SECRETS").unwrap_or_else(|| "local-encrypted".into()).as_str() {
        ...
        let cfg = Config { server, database, auth, storage, secrets, mode };
        info!(host = %cfg.server.host, port = cfg.server.port, mode = ?cfg.mode, "config loaded");
        Ok(cfg)
    }
}

fn parse_or<T: std::str::FromStr>(lookup: &dyn Fn(&str) -> Option<String>, key: &'static str, default: T) -> Result<T, ConfigError>
where
    T::Err: std::fmt::Display,
{
    match lookup(key) {
        Some(s) => s.parse::<T>().map_err(|e| ConfigError::InvalidEnv(key, e.to_string())),
        None => Ok(default),
    }
}
...
mod tests {
    ...
    fn missing_required_url_returns_error() {
        ...
        assert!(matches!(err, ConfigError::MissingEnv("PAPERCLIP_DATABASE_URL")));
    }

    #[test]
    fn invalid_port_returns_error() {
        let l = lookup_with(&[("PAPERCLIP_DATABASE_URL", "postgres://localhost/test"), ("PAPERCLIP_PORT", "not-a-number")]);
        let err = Config::build_with(&l).unwrap_err();
    ...
    fn production_mode_recognized() {
        let l = lookup_with(&[("PAPERCLIP_DATABASE_URL", "postgres://localhost/test"), ("PAPERCLIP_MODE", "production")]);
        let cfg = Config::build_with(&l).unwrap();
