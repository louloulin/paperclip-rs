...
pub struct adapter_opencode_localAdapter;

impl adapter_opencode_localAdapter {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl Default for adapter_opencode_localAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Adapter for adapter_opencode_localAdapter {
    fn descriptor(&self) -> AdapterDescriptor {
        AdapterDescriptor::builtin(ADAPTER_TYPE, "OpenCode")
    }
    ...
    async fn execute(
        ...
        result.provider = Some("opencode_local".into());
        result.model = model;
...
mod tests {
    ...
    fn descriptor_returns_correct_type() {
        let adapter = adapter_opencode_localAdapter::new();
        assert_eq!(adapter.descriptor().adapter_type, "opencode_local");
    }
