...
pub fn parse_opencode_stream_json(stdout: &str) -> ParsedOpenCodeStreamJson {
    ...
    for raw in stdout.lines() {
        ...
        if !session_id.is_empty() && parsed.session_id.is_none() {
            parsed.session_id = Some(session_id);
        }
        match string_field(&event, "type").as_str() {
                ...
                if let Some(part) = event.get("part").and_then(Value::as_object) {
                    if let Some(tokens) = part.get("tokens").and_then(Value::as_object) {
                        ...
                        parsed.usage.output_tokens +=
                            number_field(tokens, "output") + number_field(tokens, "reasoning");
            ...
            "tool_use" => {
                if let Some(part) = event.get("part").and_then(Value::as_object) {
                    if let Some(state) = part.get("state").and_then(Value::as_object) {
                        if string_field(state, "status") == "error" {
                            let text = string_field(state, "error");
                            if !text.is_empty() {
                                parsed.tool_errors.push(text);
                            }
                        }
                    }
                }
            ...
            "error" => {
                let text = error_text(event.get("error").or_else(|| event.get("message")));
                if !text.is_empty() {
                    errors.push(text);
                }
            }
...
pub fn is_opencode_unknown_session_error(stdout: &str, stderr: &str) -> bool {
    ...
    ["unknown session", "session ", "notfounderror", "no session"]
        .iter()
