//! R428 — opencode-local 端到端验证「resume 失败 → 真实重跑」主回路。
//!
//! 复刻 Node `packages/adapters/opencode-local/src/server/execute.ts` 中
//! `initial = runAttempt(sessionId); isOpenCodeUnknownSessionError → runAttempt(null)` 回路：
//! - 首轮带 `--session <sid>`：失败时返回 unknown-session 错误；
//! - 触发重试，第二轮 **不带** `--session`，必须成功；
//! - `result_json.retriedAfterUnknownSession == true` 且拿到新 session id。
...
#[tokio::test]
async fn unknown_session_triggers_real_retry_without_session() {
    let tmp = TempDir::new("opencode-retry");
    let log = tmp.path.join("calls.log");
    let body = format!(
        "#!/bin/sh\necho \"$@\" >> '{log}'\nN=$(wc -l < '{log}' | tr -d ' ')\nif [ \"$N\" = \"1\" ]; then\n  echo 'unknown session id: sess-a' >&2\n  exit 1\nelse\n  printf '%s\\n' '{{\"type\":\"text\",\"sessionID\":\"sess-fresh\",\"part\":{{\"text\":\"Recovered\"}}}}'\n  exit 0\nfi\n",
        log = log.display()
    );
    ...
    let result = OpencodeLocalAdapter::new()
        .execute(
    ...
    assert!(calls[0].contains("--session"), "首次应带 --session: {}", calls[0]);
    assert!(calls[0].contains("sess-a"), "首次应带 session id: {}", calls[0]);
    assert!(!calls[1].contains("--session"), "重试必须去掉 --session: {}", calls[1]);
...
async fn unrelated_failure_does_not_retry() {
    let tmp = TempDir::new("opencode-rate");
    let log = tmp.path.join("calls.log");
    let body = format!(
