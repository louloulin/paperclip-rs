//! R425 — opencode-local 端到端校验 `paperclipEnvNote` 与 `apiAccessNote` 注入。
...
async fn result_json_carries_prompt_notes() {
    ...
    let stdout_lines = [r#"{"type":"session","session_id":"s-1"}"#];
    let cmd = write_mock_cli(&tmp.path, "opencode-mock", &stdout_lines);
