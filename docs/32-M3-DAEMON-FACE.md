...
> **号段说明（先撞号后重编，逐字登记）**：起手复核 `docs/32` 末号 = **`## 35.`**（M7-18 / PR #110），
> `## 30.` / `## 32.` 已由 M7-14 / M7-13 占用 ⇒ 本片按 rev 2 的取值写了 **`## 36.`**。
> 但 **`LUM-1980` 的 PR #112 先落地**（23:2xZ 合入 `b248e02b`），它也写了 `## 36.` ⇒ **同号被两片占用**。
> 按 `docs/37` §129.10 的号段纪律（**号由「落地顺序」定，不是派发顺序**）⇒ 本片**重编为 `## 37.`**
> （标题 + 本文件与 `crates/mc-channel/src/wecom/**` 里 10 处自引用一并改），并入 base 后复核
> `git show <base>:docs/32-M3-DAEMON-FACE.md | grep '^## ' | tail -4`。
> 后续：**M7-20 = `## 38.`**、**M7-21 INT = `## 39.`**。
