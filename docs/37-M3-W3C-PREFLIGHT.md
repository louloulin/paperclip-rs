...
## §128 06:30 cycle（`LUM-2077`，22:30Z 触发）：**并发 cycle 并存 ⇒ 让出不可重复动作、只做独立复核** —— 独立复现 PR #111 判据链 ①②③（落地树 `dff477cb` 逐字）+ 🔴 **门 ⑥ 的 CI 红有两个「未登记」签名，且分属两个不同类**（`mc-scheduler/tests/lease_db.rs:273` = **时钟桶跨界**（新类，**产品不可达**）／`jobs_plugin_hook.rs:427` = **弱等待**（`LUM-1980` 家族））

### §128.1 起手（三连 + 逐 PID 拆槽）：**并发 cycle 连续第 2 轮**

| 读数 | 实测（22:30–22:32Z） |
| --- | --- |
| `df -h /` | **35G 可用（26%）** —— 见 §128.7 lesson 6：**12 分钟后同一盘只剩 13G（74%）** |
| `git ls-remote origin feat/multica-rs-initial` | **`aa6dbd64`**（= §127 的合并落地提交）；22:31:35Z 后变 **`9f298f0a`**（并发 cycle 的 §127 docs 直推） |
| 认证 GH `pulls?state=open` | **0**；`pulls/111` = `merged:true` / `merged_at 22:28:35Z` / `merge_commit_sha aa6dbd64` |
| daemon `running_task_count` | 起手 **4**（⬆ 超「一次最多三个任务」的硬约束）→ 22:42Z **3**（并发 cycle 终态） |
| 逐 PID（`/proc/*/cwd` + `cmdline`） | `22090` = `lum-1784`（55m）∥ `35156` = `lum-1980`（9m，22:30:34Z 起 run）∥ `35451` = **本 run** ∥ `46467` = `lum-2072`（30m → 22:4xZ 消失）；下挂 `rustc ×3` + `cargo ×2` = 活跃冷建 |

### §128.2 在飞两片体检：**两片皆活、皆未推分支、皆无 PR**

| 片 | workdir | HEAD | 提交 | `porcelain` | target | 判读 |
| --- | --- | --- | ---: | ---: | ---: | --- |
| `LUM-1784`（M7-19 wecom 入站/解析，0 路由） | `lum-1784-80429b9e50ef` | `b232f7e4`（== 起手点） | **0** | **14** | 3.3G → **12G** | 活：`wecom/**` 新文件 + `mod.rs` **26 个 `pub mod`**（§126 派的 rev 4） |
| `LUM-1980`（门 ⑥ 测试侧竞态 6 条，纯测试支撑片） | `lum-1980-e1cf9b39fad4` | `aa6dbd64` | **0** | **6** | **13G** | 活：改动的**恰好是名录六个落点**（`apps/mc-server/src/scheduler/tests.rs`、`mc-composio/src/state.rs`、`tests/channels/{telegram.rs,telegram_round_trip.rs}`、`tests/composio/{flows.rs,support.rs}`）⇒ 与 §128.4 的新签名**无交集**（它不会顺手修到） |

### §128.3 独立复核 PR #111 判据链（与 §127.3 **各自独立、逐字一致**）+ 两条口径修正

| 步 | 判据 | 本 run 实测（**我自己重取**） |
| --- | --- | --- |
| ① | `merge-base..head` numstat == PR API **逐字** | `git diff --numstat 7eb95caf a250aec9`（`sort -k3`）与 `pulls/111/files`（同序）**20 文件 / +7509 / −78 两侧逐字相等** ✓ |
| ② | 形态判定 | `7eb95caf` **是** `a250aec9` 的祖先（head 把 base 并了进来）⇒ **合并平凡**；`aa6dbd64` parents = (`7eb95caf`, `a250aec9`) ✓ |
| ③ | 树等式（三读数） | `git merge-tree --write-tree 7eb95caf a250aec9` == `a250aec9^{tree}` == **`aa6dbd64^{tree}`** = `dff477cb41d7205d09894ecd88006bacc0ba06de` **逐字命中** ✓ |
| ④ | CI 画像 | head `a250aec9`：`fast`✓ `contract`✓ **`db`✗**；base `7eb95caf`：`fast`✓ `contract`✓ **`db`✗**（**签名不同**，见 §128.4）⇒ 两棵树同形 ⇒ 与片无关 |

**口径修正 1（改写 §127.3④ 的归因）**：`refs/pull/N/merge` **不是**「因为 PR 有冲突才算不出」。我 `git fetch origin 'refs/pull/111/merge:…'` 得到 `fatal: couldn't find remote ref` —— 而 #111 **已经合并**。正确口径：**该 ref 是 open PR 的临时 ref，PR 一合即消失**（#110 当时可取，是因为它那时还开着）⇒ §126 的「四读数」**只在 PR 处于 open 时可得**；合完再回去取必然落空，与冲突无关。

**口径修正 2**：`scripts/gates.sh:235` 的 ⑥ 是 `cargo test -p mc-repos -p mc-http -p mc-scheduler -p mc-server … -- --ignored`，**没有 `--no-fail-fast`** ⇒ cargo 在**第一个失败的 target** 就停 ⇒ **每次 CI db 红只暴露一个签名**。证据（同一作业的日志行数）：红在 `mc-http/…/telegram.rs:273` 的那轮只有 **4 行 `test result:`**（`mc-scheduler`/`mc-server` 的 target **根本没跑**）；红在 `mc-scheduler/…/jobs_plugin_hook.rs:427` 的那轮有 **38 行**。⇒ **按 CI 给某签名计频会系统性低估**，且 mc-http 的红会**遮住** scheduler 族的红。

### §128.4 🔴 新发现：门 ⑥ 的 CI 红有两个**未登记**签名，分属**两个不同的类**

近 8 轮把 `feat/multica-rs-initial` 的 db 作业日志全量拉下来，按 `panicked at <file>:<line>` 归类：

| run | head | db 红签名 | 名录 |
| --- | --- | --- | --- |
| 36193307049 | `7eb95caf`（**base 自身**） | `telegram.rs:156` + `:273` | ④ ✓ |
| 36189705660 | `8c42f28b` | `telegram.rs:273` | ④ ✓ |
| 36187028216 | `cd5bc617` | `telegram.rs:273` | ④ ✓ |
| 36186465768 | `06d2f4bf` | `composio/support.rs:577` | ①③ ✓ |
| 36184012307 | `7acc2d84` | `telegram_round_trip.rs:599` | ② ✓ |
| 36177464025 | `33d476e5` | `telegram.rs:273` | ④ ✓ |
| 36174218750 | `0bb943f2` | **`mc-scheduler/tests/jobs_plugin_hook.rs:427`** | **未登记** |
| — | `a250aec9`（PR #111 head） | **`mc-scheduler/tests/lease_db.rs:273`** | **未登记** |

#### (a) `crates/mc-scheduler/tests/jobs_plugin_hook.rs:427` —— **弱等待类**（= `LUM-1980` ②③④ 同族，可直接并入）

- 用例 `real_db_the_loop_delivers_each_schedule_bucket_exactly_once`；现场 `assert_eq!(latest.status, ExecutionStatus::Success)`（CI `427:13`，`left == right` 失败）。
- 机理（读码）：循环体是 `if latest.found { assert_eq!(status, Success); break; }`（`:426-428`）—— 而 `process_plan` 是**先 `try_claim` 写 RUNNING、再跑 handler、最后写终态**（`manager.rs` `process_plan` → `write_success`）⇒ **只要在「行已插入、handler 还没写完」之间被观测到，断言立刻红**。handler 是桩端口（亚毫秒），轮询间隔 50ms ⇒ 命中率 ≈ 毫秒/50ms ≈ **2%**（该轮 1/1 命中，另 7 轮未命中，与 2% 量级相符）。
- **最小修法（只改测试）**：等待条件从「行存在」收紧为「**行已终态**」——`if latest.found && latest.status != ExecutionStatus::Running { assert_eq!(…, Success); break; }`；或统一成 `wait_for(|| latest.status == Success)` + 超时。

#### (b) `crates/mc-scheduler/tests/lease_db.rs:273` —— **时钟桶跨界类**（**新类**：不是弱等待、不是共享状态、不是数据相关）

- 用例 `every_plan_returns_to_the_failed_bucket_when_its_backoff_has_burned`；现场 `assert_eq!(attempts[0].0, attempts[1].0, "FAILED 桶还会重试 ⇒ 游标不能跳过它")`（CI `273:5`，第 **2** 条断言；第 1 条「两个 tick 各跑一次」`len==2` **通过了**）。
- **确定性条件（读码得出，可逐字对上现场）**：该用例的 job 配置是 `cadence=60s`、`schedule_delay=0`（`with_timing` 只设 run/stale/heartbeat）、`catch_up_window=`**`Duration::zero()`**、`max_plans_per_tick=1`。
  - `plans_for_tick`：`latest = floor_plan(now - schedule_delay, cadence)` ⇒ 桶 = **UTC 分钟**。
  - tick 1（无历史）：`start = latest1 = floor_plan(T1)` ⇒ handler 以 `plan_time=latest1` 失败，`next_retry_at = T1`（空退避表 ⇒ 立刻可重试）。
  - tick 2：`retry_eligible(T2)` 为真 ⇒ 规则 1 令 `start = info.plan_time = latest1`（`manager.rs:271-276` 自述：否则那个 FAILED 桶会被永久跳过）；**但**窗口 ≤ 0 ⇒ `oldest_allowed = latest2`，随后的 `if start < oldest_allowed { start = … }` **把 latest1 夹掉**、换成 `latest2`。
  - ⇒ 当且仅当 `floor_plan(T1,60s) < floor_plan(T2,60s)`（**两次 `run_once()` 跨过分钟边界**）时，第 2 个 tick 跑的是**新桶** ⇒ `attempts[0].0 ≠ attempts[1].0` ⇒ **必红**（第 3 条断言 `(1,2)` 也会红，只是 273 先触发）。窗口 = 两次 tick 的间隔（几十~几百 ms）/60s ≈ **0.1–1%**。
- ⇒ 这是**时钟依赖的确定性失败**，不是「概率抖动」；且**只在该测试的配置下可达**：三个生产 job **全是 `CatchUpMode::LatestOnly`**（`plugin_hook.rs:445`、`issue_wakeup.rs:277`、`autopilot.rs:609`），前两个还带 `PlansHook`（`catch_up_mode` 只当审计）⇒ `EveryPlan + window=0` 的组合**生产不可达** ⇒ **不是产品回归**，不拦合并。
- ⚠️ 但**语义冲突值得登记给 M5 代码面**：「窗口 0 ⇒ 只认最新桶」与本模块**自述的规则 1**（`manager.rs:271-275`：还能重试的 FAILED 行必须停在同一个 `plan_time`）冲突，实现选了**窗口赢**。修法二选一：① 测试侧改成正窗口（如 `Duration::minutes(5)`）——规避开窗，仍能钉住规则 1；② 产品侧让规则 1 压过窗口（需 M5 spec 仲裁，且会让该用例变成 **1% 概率的确定红**，反而更糟）。
- **本轮建议**：按 ① 修（测试侧一行 + 注释），把「窗口 0 会吞掉可重试 FAILED 桶」作为**已知语义**记入 `docs/48`。

### §128.5 ⑦ / ⑩ 当场；⑨ 改用**静态差异复核**（本轮刻意不跑重型 cargo）

- **⑦ PASS（4s）**：`upstream 456 (f41fae6b08fb) | local 474 registered | baseline 458 | implemented 388 real + 3 placeholder = 391 / 456 | known_gap 65 | unclaimed 0 | regression 0 | local_only 9` + `OK: every upstream route is either implemented or owned` ⇒ **与 §127.6 逐字相同**（独立复现）；`implemented 386 → 391` 的 **+5** 恰为 M7-14 的 5 条 lark 路由，`owners.M7 = 0`。
- **⑩ PASS（1s）**：`scanned 1114 / baseline 10 / violations 0`（未动）。
- **⑨ 不跑二进制**（理由：磁盘已从 35G 掉到 13G，两片正在冷建，重跑一次全量就是 §127.4 的 ENOSPC 事故）⇒ 改用 **report.json 叶子级静态 diff**（`7eb95caf` vs `aa6dbd64`）：
  - 5140 个叶子中**恰好 39 个变化**（== 该文件的 numstat `+39/−39`），**全部落在恰好 7 条 fixture**（下标 **280 / 330 / 344 / 345 / 346 / 347 / 352**，即 `lark_test.go` 的 7 个 `*NotConfigured*` 用例），全部 `outcome: unmounted → pass`（`offline` 同步转 `pass`），`status_observed` 404 → 403/200 与 `status_expected` 相符；
  - `totals.pass 7 → 14`、`totals.unmounted 29 → 22`（**逐字命中**片自报）；`contract_equivalence_rate 0.0192 → 0.0384`；
  - **无新增/删除 fixture**（两侧叶子数相同）、**无其它 fixture 被碰**（`only-in-before`/`only-in-after` 均空）⇒ 片自报的「**差异集 == 本片 fixture 集**」**成立**。
  - ⚠️ 口径：这仍是**一手证据**（核的是同一份 `report.json`），只是「验法」从跑二进制换成静态 diff —— **磁盘紧轮的省法，第 1 次用**。

### §128.6 槽位与派发：**零派发**（3/3 满位）；**不改任何不可重复动作**

- 起手 **4/3**（两个 cycle 并存，越限）→ 22:42Z 降回 **3/3**（`LUM-2072` 终态）⇒ 无空位。
- **本窗口的不可重复动作全部由并发 cycle `LUM-2072` 完成**：合并 **PR #111**（22:28:35Z）、docs 直推 **§127**（22:31:35Z）、派 **`LUM-1980`**（22:30:34Z）⇒ 本 run 只做**可重复**动作（独立复核 / ⑦⑩ / 静态 ⑨ / 文档登记 / 回收评估）。
- **回收 = 0**：`LUM-1779`（已合并、run 终态）的 `target/` 已被 §127 整删，只剩 **104M** 源码；其余 12 个历史 workdir 各 ~27M（平台管理）⇒ 无可回收量（PG 侧总量仅 **599M**，也无可回收量）。
- 🔴 **磁盘（本轮最硬约束）**：起手 35G → 12 分钟后 **13G（74%）**，消耗方**全是别人的合法副作用**（1784 的 target 3.3G→12G、1980 的 target →13G），**本 run 一个 cargo 都没跑**。下一轮起手必须先算「可用 − 在飞两片 ≈25G」再决定是否跑 `--with-db`。

### §128.7 lesson

1. **并发 cycle 的让位规则要写死**：两个 cycle 同窗口时，**先到者**拿走「合并 / 派发 / 写本窗口 §N」三件**不可重复**动作，**后到者**只做可重复动作（独立复核 + 快照 + 登记 + 回收评估）。若后到者照抄前任的 `next cycle 起点` 行去「合并 PR #111 / 派 LUM-1980」，会**连撞两次**。
2. **`refs/pull/N/merge` 是 open PR 的临时 ref**（合并即消失）⇒ §127.3④ 把它归因给「冲突」是**误判**；要取「四读数」必须在**合并前**。
3. **`cargo test` 默认 fail-fast**（`gates.sh:235` 无 `--no-fail-fast`）⇒ CI 每次只报**第一个**失败 target ⇒ 拿 CI 当签名名录会**漏**（scheduler 族被 mc-http 的红遮住）。
4. **CI 红必须分类，不能一律归「概率门」**：本轮两个新签名，一个是弱等待（同族、可并入），一个是**时钟桶跨界**（新类、生产不可达）——归错类会把「确定性时钟依赖」当「概率抖动」，修法完全不同（前者改断言、后者改配置）。
5. **⑨ 可以静态验**：`report.json` 叶子级 diff（5140 叶 → 39 叶 → **恰好 7 条 fixture**）能**逐字**复现「差异集 == 片 fixture 集」，**零构建**。
6. **起手 `df` 依然是过期读数（第 4 次）**：35G(26%) → 13G(74%)，**22G 全是别人冷建的合法副作用**（§127 lesson 同样撞过；本 run 甚至什么都没跑）。

### §128.8 观察项（第 65 轮）

- 🔴 **并发 cycle 连续第 2 轮**（`LUM-2072` 22:00Z ∥ `LUM-2077` 22:30Z）；autopilot 护栏（「同项目存在未终态 cycle issue 时不建新单」）**仍未落地**；起手 `running_task_count = 4` 已**越限**。
- 积压 `todo` cycle **只登记、不动状态**（本轮不动任何别的 issue 状态）。
- 看板精确计数**取不到**：`issue list` 单页上限 50，逐状态返回 `in_review 50 / todo 50 / in_progress 41 / backlog 50 / blocked 6` —— **那是分页上限，不是真值** ⇒ 以后不要把这种读数当判据写进文档（§127.9 的 `239+/13/…` 同此性质）。

### §128.9 next cycle 起点（07:00 / 23:00Z）

- **base = `9f298f0a`**（+ 本 §128 docs-only 直推）；GH **0 open PR**；daemon **3**（cycle 自身 ∥ `LUM-1784` ∥ `LUM-1980`）。
- **槽位（勿抄旧行）**：`1784` 终 ⇒ **`LUM-1785`**（M7-20 rev 4；**同一 `wecom/mod.rs` 追加段，必须等 1784 合入**）；`1980` 终 ⇒ **`LUM-1786`**（M7-21 INT rev 5；`owners.M7 = 0` 已成立，但须等 1784/1785 收口，且不得与任何 `--write-baseline` 片同轮）。
- **起手第一动作**：`df -h /` 连采两次（**先减去在飞两片 ≈25G**）→ `git ls-remote` → GH `pulls?state=open` → 逐 PID 扫 `/proc/*/cwd` → 逐片终态判定 → 才进判据链。
- **门 ⑥ 红的新分类流程**：① 先按 §128.4 表查签名（`telegram.rs` / `telegram_round_trip.rs` / `composio/**` = 名录；`jobs_plugin_hook.rs:427` = 弱等待；`lease_db.rs:273` = 时钟桶跨界）② 落在写集外 ⇒ 换**当轮新建库**复验 ③ 仍红再查 `df`（`os error 28`）。

## §129 07:00 cycle（`LUM-2082`，23:00Z 触发）：**起手 0 open PR + 空位 0（3/3 满位）+ 🔴 磁盘 6.2G / 87% ⇒ 只读监控轮（第二十五次）**；⑦ 当场逐字复现 §128、⑩ **`scanned 1130`（订正 §128 抄来的过期值 1114）**、⑨ 用「门输入逐 blob 恒等」**第 8 次主动不冷编**；零空位不放空 ⇒ 两片递补 **rev bump**（`LUM-1785` 4→5 / `LUM-1786` 5→6）；🔴 **新证据：`LUM-1980` 的工作树已把 §128 新抓的两条签名并入本片（名录 5 条 ⇒ 实为 7 条），且它当场在做「先复现（`TEMP-EXPERIMENT`）再修」的对照实验**；回收 **+563 MiB**（**首次回收 PG 真库：50 个库 → 5 个**）

### §129.1 起手（三连 + 逐 PID 拆槽）：**无并发 cycle（连续第 19 轮）**

| 读数 | 实测（23:00–23:05Z） |
| --- | --- |
| `df -h /` | **6.2G 可用（87%）** 23:00Z → `5.4G`（89%）→ `5.1G`（90%）→ **3.7G（93%）** 23:0xZ ⇒ **15 分钟内 −2.5G，全是两片增量编译的合法副作用**（本 run 未跑任何 cargo） |
| `git ls-remote origin feat/multica-rs-initial` | **`f3c92913`**（= §128 的 docs 直推）；到 23:05Z **未前进** |
| 认证 GH `pulls?state=open` | **0**（23:00Z / 23:03Z / 23:04Z **三次**实测都是 0） |
| daemon `running_task_count` | **3**（`active_task_count = 3`）⇒ **未越限**（§128 起手的 4/3 已回落） |
| 逐 PID（`cmdline` + `/proc/*/cwd`） | `22090` = `lum-1784`（etime 1h21m）∥ `35156` = `lum-1980`（etime 32m）∥ `56030` = **本 run**；下挂 `65347 → 65486`（1784 的 cargo/rustc）+ `60318 / 62731…`（1980 的 `cargo test -p mc-scheduler --no-run`）⇒ **两片都在编译** |
| 空位 | **3 − 1(cycle) − 2(在飞) = 0** |

### §129.2 在飞两片体检：两片皆活、皆 **0 提交**、皆未推分支

| 片 | workdir | HEAD | 提交 | `porcelain` | target | 判读 |
| --- | --- | --- | ---: | ---: | ---: | --- |
| `LUM-1784`（M7-19 wecom 入站/解析，0 路由） | `lum-1784-80429b9e50ef` | `b232f7e4`（== 起手点） | **0** | **14 → 21** | 3.3G → 12G → **29G** | **活**：分钟级在改 `wecom_channel/tests/{round_trip,doubles}.rs` + `tests.rs`，正解 **E0583**（`file not found for module doubles`）/ **E0603**（`CountingTrigger is private`）= 测试模块拆分与可见性 |
| `LUM-1980`（门 ⑥ 测试侧竞态，纯测试支撑片） | `lum-1980-e1cf9b39fad4` | `aa6dbd64` | **0** | **8**（`+112 −33`） | 4.2G | **活**：`cargo test -p mc-scheduler --no-run` 编译中 |

**`LUM-1784` 已跑过一轮收尾段并自恢复过一次 ENOSPC（一手证据来自它自己的 session）**：

- 它自己的 `--with-db` 曾以 **`GATE_DB_E2E_EXIT=101` + `error: could not create incremental compilation session directory …`** 红 —— 正是 §127.4/§128 的 **ENOSPC 形态**（不是代码红）；随后同一条 session 里 `df` 从 **`0 / 100%`** 变回 **`21G / 56%`**（它自己回收了别人的死物），重跑 ⇒ **`GATE_DB_MIGRATE_EXIT=0` / `GATE_DB_E2E_EXIT=0` / `GATE_DB_EXIT=0`**、`report matches crates/mc-conformance/report.json`、`GATE_CONFORMANCE_EXIT=0`、⑩ 触达 ⇒ **门禁段已过一次**。
- ⚠️ 但它随后**又回到测试编译迭代**（上面的 E0583/E0603）⇒ **尚未提交**。⇒ 本轮**不进判据链**（无 PR 可判）。
- 口径：`LUM-1784` 的 `porcelain` 从 14 → 21，且**只动 `wecom/**`**（14 项全是 `wecom/` 下的新文件与 `mod.rs`/`outbound*` 的追加）⇒ 与 lark 树零交集。

**🔴 `LUM-1980` 的名录已从 5 条扩到 7 条（§128 新抓的两条被它自己并了进来）**：

- 它的 `git diff --stat` = **8 文件 `+112 −33`**，其中**两条正是 §128 判定「未登记」的签名**，且修法与 §128.4 的建议**逐字一致**：
  - `crates/mc-scheduler/tests/jobs_plugin_hook.rs`：`if latest.found {` ⇒ **`if latest.found && latest.status != ExecutionStatus::Running {`**（**弱等待类**，§128.4(a) 的建议）；注释里已引 `docs/32` §36.7。
  - `crates/mc-scheduler/tests/lease_db.rs`：`.with_catch_up(CatchUpMode::EveryPlan, Duration::zero(), 1)` ⇒ **`Duration::minutes(5)`**（**时钟桶跨界类**，§128.4(b) 的建议①）；注释里已引 `docs/32` §36.8 / `docs/48` §8。
- 🔴 **并且它当场在做对照实验**：`lease_db.rs` 里有一个 **`TEMP-EXPERIMENT:` 块** —— `let secs = 60 - Utc::now().second() + 1; sleep(secs)` 把第二个 tick **推到下一个分钟桶**，用来在**修复前**复现 base 的跨桶必红。这是本项目**第一次**看到「测试修复片自带复现实验」的证据形态（修复前复现 ⇒ 修复后转绿），**但它必须在提交前删掉**。
- ⇒ 交付时该片应把名录订正为 **7 条**（rev 7 描述写的是「5 条」）。

### §129.3 ⑦ / ⑩ 当场；⑨ 静态继承（第 **8** 次）

- **⑦ PASS（1s）**：`upstream 456 (f41fae6b08fb) | local 474 registered | baseline 458 | implemented 388 real + 3 placeholder = 391 / 456 | known_gap 65 | unclaimed 0 | regression 0 | local_only 9`；`gaps by owner = {M9:33, M3+:16, M3:11, M10:5}`（和 = 65 ✓）⇒ **与 §128.5 逐字相同**（独立复现）。
- **⑩ PASS（1s）**：`scanned **1130** / baseline 10 / violations 0` —— 🔴 **订正 §128.5 的 `1114`**：那是 **#111 合入之前**的读数，§128 写它时用了「未动」这一**未实测的假设**。当轮实证：`git tree` 层面 `7eb95caf` → `aa6dbd64` 的 tracked 文件数 **2190 → 2206 = +16**，且这 16 个新增**全是 `crates/mc-channel/src/lark/**.rs` 与 `crates/mc-http/src/routes/channels/lark/**.rs`**（`--diff-filter=A` 逐条列出），**全部落在 ⑩ 的扫描口径** `("crates/", (".rs",))` 内 ⇒ `1114 + 16 = 1130` **逐字吻合**。
- **⑨ 不冷编（第 8 次生效）**：三个门输入与 §128 的验证树 `aa6dbd64` **逐一恒等** —— `crates/mc-conformance/report.json` blob **`3eb0430a39c8cf6c50ccafe626542abba3eaa0d4`**、`docs/fixtures` tree **`5e44cecddfd9def061107d243af379f4d988c42f`**、`crates/mc-conformance` tree **`f696e5ccd520631d66b55c145f4c42bc15e43ed7`**；`git diff --name-only aa6dbd64 f3c92913` = **只有 `docs/37-M3-W3C-PREFLIGHT.md`** ⇒ 读数继承：totals **`fixtures 365 / pass 14 / mismatch 23 / unmounted 22 / placeholder 0 / unevaluable 306`**（本 run 直接从 `report.json` 的 `totals` 读出，未跑二进制）。

### §129.4 槽位与派发：**零派发**（3/3 满位）

- 空位 = 0（§129.1）⇒ 不派任何片；本 run 也**不改任何在飞片的状态**（`LUM-1784` / `LUM-1980` 都已 `in_progress`）。
- 零空位不放空 ⇒ 把两片**递补**的读数刷新到当轮（`LUM-1785` rev 4 → **5**、`LUM-1786` rev 5 → **6**），内容 = 当轮 ⑦/⑨/⑩ 实测 + 写者链/硬前置/号段的当轮复核 + 一条**订正**（`LUM-1785` rev 4 的 ⑩ `1114` 作废 ⇒ `1130`）。

### §129.5 回收 = **+563 MiB**（🔴 新杠杆：PG 真库；**首次不碰任何 `target/`**）

- **动作**：删掉 **45 个过期真库**（判据 = 名字里不含在飞片的 run-id ∧ `pg_stat_activity` 里零连接），保留 `multica_lum1784` / `mc_lum1980` / `postgres` / `template0` / `template1`。
- **读数**：`df --output=avail /` **3746 MiB → 4310 MiB**（**+563 MiB**，量只认 `df` 前后差）；PG 侧 **50 个库 / 625 MB → 5 个库 / 63 MB**。
- **为什么这是本轮唯一可动的杠杆**：两个在飞片的 `target/`（**29G** + **4.2G**）都是**活物**（都在编译）；其余 12 个历史 workdir 各 ~30M（平台管理）；`.repos` 的 1.7G 跨项目裸库镜像**只登记不删**。⇒ PG 真库是**唯一一个既不碰 target、也不碰在飞进程**的回收面；在**磁盘第一风险**的轮次里应排在「外科切 incremental」**之前**（incremental 会打到活构建）。
- 与 §127 的「删 12 个真库 2372M → 562M」同源，但当轮总量只剩 625M ⇒ 收益上限就是几百 MB。

### §129.6 lesson

1. 🔴 **「未动」是一个必须实测的断言，不是一个可以继承的假设**：§128 的 ⑩ 把上一轮的 `1114` 抄了下来（理由「未动」），而当轮实测是 **`1130`**（#111 新增 16 个 `.rs`）。**更正手法**：用 `git ls-tree -r --name-only <before> | wc -l` vs `<after>` + `--diff-filter=A` 逐条列出新增文件，再对照门自身的 `SCOPE` 前缀/后缀 ⇒ 一次就能证明「+16 且全在口径内」。
2. 🔴 **回收的排序键要按「是否碰到活物」排，而不是按体量排**：本轮最大的两块（29G / 4.2G）都在活构建，**一字节都不能动**；而 **563 MiB** 的 PG 真库是**唯一零风险面**。⇒ 磁盘轮的回收顺序 = ① PG 真库（零接触）→ ② 死物 `target/`（四判据）→ ③ 活物 `incremental`（>10min 且无 fd 引用）。
3. 🔴 **修复片自带「先复现再修」的对照实验**（`LUM-1980` 的 `TEMP-EXPERIMENT:` 块把第二个 tick 推到下个分钟桶去复现时钟桶跨界）⇒ 这是比「改完就绿」强得多的证据形态，值得作为**测试修复片的验收期望**写进名录；同时提醒：**`TEMP-EXPERIMENT` 必须在提交前删除**（否则把 60s 睡眠带进 CI）。
4. **`porcelain` 项数可以当活性指标，但不能当进度指标**：`LUM-1784` 从 14 → 21 项，而它其实**已经跑过一整轮门禁**（db/conformance/⑩ 全绿）又**回到**测试编译迭代 ⇒ 「项数在涨」只说明还活着。判「是否在收尾」要看 session 里的**门禁读数**（`GATE_*_EXIT`）。
5. **ENOSPC 的自恢复已在第 3 片身上复现**（`LUM-1784` 自己回收 ~21G 后重跑 db 门转绿）⇒ 「片自己会救盘」这条经验成立，但**代价是它白跑一轮门禁**；cycle 侧提前回收（lesson 2）能省掉这一轮。

### §129.7 观察项（第 **66** 轮）

- 起手 **无并发 cycle（连续第 19 轮）**；daemon `running_task_count = 3` **未越限**；autopilot 护栏（「同项目存在未终态 cycle issue 时不建新单」）**仍未落地**。
- 积压 `todo` cycle **只登记、不动状态**（本轮不动任何别 issue 的状态）。
- 看板精确计数**取不到**（`issue list` 分页上限）⇒ 不把这种读数当判据（沿用 §128.8）。
- 🔴 `docs/32` 末号仍 = **`## 35.`**（`## 36.` 预留 `LUM-1784`、`## 37.` 预留 `LUM-1785`）⇒ 已同步进两片 rev。

### §129.8 next cycle 起点（07:30 / 23:30Z）

- **base = `f3c92913` + 本 §129 docs-only 直推**（起手一律 `git rev-parse` 实测，别抄本行）；GH **0 open PR**；daemon **3**（cycle 自身 ∥ `LUM-1784` ∥ `LUM-1980`）。
- **起手第一动作**：`df -h /` **连采两次**（**先减去在飞两片 ≈33G 的在建增量；可用 < 4G 就先回收再想别的**）→ `git ls-remote origin feat/multica-rs-initial agent/devbox5/*` → 认证 GH `pulls?state=open` → **从 `/` 起手**逐 PID 扫 `/proc/*/cwd`（先读 `cmdline` 是不是 `pi`）→ 逐片「run 终态（`porcelain` 空 + pi 进程消失 + 提交数/是否已推）∧ 形态判定」→ 终态才进判据链。
- **判据链铁律（沿用）**：① 预检 `merge-base..head` numstat == PR API 逐字 ② 形态两半（`merge-base == base` ∧ 尾提交 docs-only）/形态③ ⇒ **真合 + 在片自己的热 target 上重跑 `--with-db` 10/10**（`LUM-1784` 的 29G target 是热物，**勿清**；`LUM-1785` 一旦起手就与它争 `wecom/mod.rs`，故 1785 只能在 1784 **合入之后**派）③ 三/四读数（`merge-tree --write-tree` == rehearsal `write-tree` == `refs/pull/N/merge^{tree}` == 落地 `^{tree}`；⚠️ `refs/pull/N/merge` **只在 PR open 时存在**）④ 证据两条路任一：head CI 3/3 绿 **或** 合并树同树 `--with-db` 10/10 ⑤ API 钉 40 位 sha + `merge_method=merge` ⑥ 落地树逐字 + `git diff` 空。
- **门 ⑥ 红先分类**（`docs/37` §128.4 表）：`telegram.rs` / `telegram_round_trip.rs` / `composio/**` = 名录；`jobs_plugin_hook.rs:427` = 弱等待；`lease_db.rs:273` = 时钟桶跨界 —— **后两条正是 `LUM-1980` 本轮在修的**，它合入后这两条应消失；仍红就先查 `df`（`os error 28`）+ 换**当轮新建库**（角色带 `CREATEDB`）。
- **槽位链（勿抄旧行）**：`1784` 终 ⇒ 判据链 + 合入 ⇒ 空位 ⇒ **`LUM-1785`**（M7-20 rev 5；**必须在 1784 合入后**起手，同写 `wecom/mod.rs` 追加段）⇒ 之后 **`LUM-1786`**（M7-21 INT rev 6；`owners.M7 = 0` **已成立**，唯一一次 `--write-baseline`：`458` → 当轮 `local` 实测；与 M9-INT `LUM-1825` **不得同轮**）。`LUM-1980` 终 ⇒ 其合入即修复 7 条测试侧竞态 ⇒ 之后门 ⑥ 的「概率门」判据要**重定基线**（旧名录不再是有效参照）。
- **回收预告**：`LUM-1784` 一合 ⇒ 其 **29G** `target/` 四判据齐即可整删（**本轮最大的单块回收**）；`LUM-1980` 一合 ⇒ 4.2G。

### §129.9 轮内转非只读：`LUM-1980` 在 23:24:00Z 交付 ⇒ 判据链合并 **PR #112**

`LUM-1980` 在本轮进行中交付：`fd85db9f`（测试修复，23:12:27Z）→ 并 base `0273b817`（= **本 run 的 §129 直推**）→ head **`3e99926f`**（23:12:35Z）→ **PR #112 于 23:24:00Z 开出**，其后 `porcelain` 归零、pi 进程在 23:3xZ 消失 ⇒ **空位 1**。判据链六步：

| 步 | 判据 | 本 run 实测 |
| --- | --- | --- |
| ① | `merge-base..head` numstat == PR API **逐字** | `git diff --numstat 0273b817 3e99926f` = **10 文件 `+337 −33`**，与 `pulls/112/files` **逐文件逐字相等** ✓ |
| ② | 形态判定 | `0273b817` **是** head 的祖先（片自己把 base 并了进来）⇒ **合并平凡** ✓ |
| ③ | 三读数 | `head^{tree}` == `git merge-tree --write-tree` == **`refs/pull/112/merge^{tree}`** = **`bcf7531c61f436e47561010ed54d9f8156be30ef`** ✓（`refs/pull/N/merge` 在 PR **open** 时可得 ⇒ 再次印证 §128.3 的口径修正「合并即消失 ≠ 冲突」） |
| ④ | 证据 | **同树** `--with-db` **10/10**（`overall: PASS — 10/10 gate(s) green in 513s`，DB `mc_lum1980`）。「与合并树同一棵树」用**时间戳**证明：末笔提交 `3e99926f` = **23:12:35Z** < 门禁完成 = **23:21:23Z**，且片工作树 `porcelain` 为零 ⇒ 那次 10/10 跑的**就是** head 树 ✓ |
| ⑤ | 钉 sha + 合并 | API `PUT /pulls/112/merge` 带 40 位 sha `3e99926f…` + `merge_method=merge` ⇒ 落地提交 **`b248e02bddc6ae573e08705988325900d4ae656c`** ✓ |
| ⑥ | 落地验证 | 落地 `^{tree}` == `bcf7531c…` **逐字命中**、`git diff 3e99926f <base>` = **0 行**；⑦ 在落地 base 上当场重跑 **2/2 绿（1s）**、九个数**逐字不变**（0 路由纯测试片，与预测一致）✓ |

- **片的名录从 5 条扩到 8 条**（PR 标题即写「八条」）：8 个测试文件含 §128 新抓的两条（`jobs_plugin_hook.rs:427` 弱等待 / `lease_db.rs:273` 时钟桶跨界），且 `lease_db.rs` 的 `TEMP-EXPERIMENT` 复现块**已在提交前删除**（`git show 3e99926f:… | grep -c TEMP-EXPERIMENT` = **0**）、`catch_up_window` 终值 = `Duration::minutes(5)`（§128.4(b) 的建议①）；另带两个 docs（`docs/32` §36 / `docs/48`）。
- **回收**：`LUM-1980` 的 `target/` **19G** 按四判据（PR 已合 ∧ run 终态 ∧ `/proc/*/cwd` 逐 PID 零命中 ∧ `porcelain` 空）整删 ⇒ `df -h /` **5.8G → 25G**。
- **本轮回收合计**：PG 真库 **+563 MiB** + `LUM-1980` 的**陈旧** `incremental` **≈1G**（`CARGO_INCREMENTAL=0` 的构建不写它；末次写入 23:06:53 ⇒ 18.7 分钟前、零 fd 引用）+ `target/` **19G**。

### §129.10 🔴 号段撞车（新 finding）+ 空位 1 **无处可派**

- 🔴 **`docs/32` 的 `## 36.` 被两片同时占用**：`LUM-1980`（**已落地** `b248e02b`）写 `## 36. 门 ⑥ 的测试侧竞态收口（LUM-1980）：八条`，而在飞的 `LUM-1784`（提交 `ec95337d`，未推分支）同样写 `## 36. M7-19（LUM-1784）：wecom 入站与解析` ⇒ **它一合并就会出现重号**（甚至同点追加冲突）。**裁定（沿用 §127 的冲突定式「两侧都保留 + 号段归位到编号单调位」）**：先落地的 `LUM-1980` 保 `## 36.`，`LUM-1784` 的重编号为 **`## 37.`** ⇒ 后续 M7-20 = **`## 38.`**、M7-21 INT = **`## 39.`**。已写进三片描述（`LUM-1784` rev 7 / `1785` rev 6 / `1786` rev 7）。
  - **lesson（第五类预飞检查 = 号段对账）**：号由**落地顺序**占，而各片在**起手时**就按「末号 + 1」写进自己的 docs ⇒ **凡有 ≥2 片同时在飞，号段必须在合并那一刻再对一次账**；本 run 是在合并 `#112` 之后用 `git show HEAD:docs/32-… | grep '^## ' | tail -4` 才发现的。
- **空位 1（`LUM-1980` 终态）但无处可派**（逐条给可观测阻塞，不是「忘了派」）：`LUM-1785`（M7-20）硬前置 = `LUM-1784` 合入 ⇒ 当轮 base 里它的 5 个产物**仍全 MISSING**；`LUM-1786`（M7-21 INT）= 等 1784+1785 收口；`LUM-1815`（M9-0）= `docs/62` §7.1 硬前置「M7 全合」未成立；`LUM-1745`（M5-D8）**已交付且已合入**（`apps/mc-server/src/webhook_worker.rs` 在 base、`main.rs` 第 10 步已接线），issue 停在 `in_review` 等人工验收 ⇒ **不是派发候选**。⇒ 空位**刻意留空**。
- **轮末 base = `b248e02b` + 本 §129.9/§129.10 docs-only 直推**（起手一律 `git rev-parse` 实测）；GH 轮末 **0 open PR**；daemon **2**（cycle 自身 ∥ `LUM-1784`）。

## §130 07:30 cycle（`LUM-2087`，23:30Z 触发）：**起手 0 open PR + 空位 1 + 磁盘健康（20G / 58%）⇒ 只读监控轮（第二十六次）**；⑦/⑩ 当场（与 §129 逐字相同）、⑨ 用「门输入逐 blob 恒等」**第 9 次主动不冷编**；🔴 **门 ⑥ 重定基线拿到「受控对照对」：`f3c92913` 与 `0273b817` 只差 `docs/37` 一个文件（+76 行、零代码）却 CI 一红一绿（#369 红 / #370 绿），且近 3 个红 run 的红点逐字取证后 100% 落在 `LUM-1980` 名录内、无未登记签名**；在飞 `LUM-1784` 的 3 条自报**独立复核全中**（⑦ 九个数 / ⑨ 三个输入 blob / ⑩ 最长新文件 773 行 + `scanned 1147`）；**合并预演：base × 1784 head 的唯一冲突面 = `docs/32`（号段），其余全自动合并**；零派发（**空位 1 无处可派**，逐条给可观测阻塞判据）；回收 +40 MB（PG 死库 2 个；`df` 粒度看不出）

### §130.1 起手（三连 + 逐 PID 拆槽）：**无并发 cycle（连续第 20 轮）**

| 读数 | 实测（23:30–23:35Z） |
| --- | --- |
| `df -h /` | **20G 可用（58%）** 23:30Z → **20G（60% used）** 23:35Z ⇒ 4 分钟内被在飞片的 ⑥ 编译吃掉的量**小于 1G**（本 run 零 cargo） |
| `git ls-remote origin feat/multica-rs-initial` | **`067329b6`**（= §129.9/§129.10 的 docs 直推，23:2xZ） |
| 认证 GH `pulls?state=open` | **0**（23:31Z） |
| daemon `running_task_count` | **2**（`active_task_count = 2`）⇒ **未越限** |
| 逐 PID（`cmdline` + `/proc/*/cwd`） | `22090` = `lum-1784`（**etime 1h49m**）∥ `6755` = **本 run**；1784 下挂着 `63388 timeout 4800 bash scripts/gates.sh --with-db` → `14378 cargo test -p mc-repos -p mc-http -p mc-scheduler -p mc-server` → `16941 autopilots-dd82…`（**已进入 ⑥ 的用例执行段，不再是编译段**） |
| 空位 | **3 − 1(cycle) − 1(在飞) = 1** |

- 🔴 **新坑（起手第一步就有坑）**：`multica repo checkout https://github.com/louloulin/paperclip-rs` 这一次落在 **`main` 线**（HEAD `4fc96f30` = 另一条历史，与 `feat/multica-rs-initial` **无共同祖先** ⇒ `git merge-base` **rc=1**、`git log origin/feat/multica-rs-initial..HEAD` 打出一整棵无关树）。**必须先** `git fetch origin feat/multica-rs-initial` + `git checkout -B agent/devbox5/<suffix> origin/feat/multica-rs-initial`（本 run 已做）——否则 ⑦/⑩/⑨ 会对着**错误的树**跑，而且**不会报错**（脚本只看工作树）。判据：起手第一件事核对 `git merge-base --is-ancestor <base> HEAD`（或直接 `git rev-parse HEAD` == `git ls-remote` 的 base）。

### §130.2 在飞片 `LUM-1784`（M7-19 wecom 入站与解析，0 路由）体检：**活、已提交、树干净、未推**；3 条自报**独立复核全中**

| 项 | 实测（23:31–23:35Z） |
| --- | --- |
| 提交 | **2 笔**（`f617bdd8` feat / `effc01cb` docs，末笔 `23:27:16Z`），merge-base = `0273b817`（比 base `067329b6` 落后 3 笔 ⇒ **它必须并一次 base**，而那次并会撞 `docs/32`，见 §130.6） |
| `porcelain` | **0**（树干净；§129 的 14→21 项已全部落成提交） |
| 分支 | `agent/devbox5/80429b9e50ef` **未推**（`git ls-remote` 空） |
| 当前动作 | `bash scripts/gates.sh --with-db`（`timeout 4800`，**已到 ⑥ 的用例执行段**）⇒ **收尾段**，不是迭代段 |
| 自报（feat 提交正文） | `--with-db` **10/10 PASS / 556s**（新库 `multica_lum1784`）、⑦ 九个数逐字不变、⑨ blob 未动、⑩ 最长新文件 773 行、`mc-channel` lib **1449 passed** |

**独立复核（零成本、三中三）**：

1. **⑦**：在**它自己的工作树**（= 已提交的 `effc01cb`，`porcelain` 为零 ⇒ 与提交树同一棵）上当场跑 `python3 scripts/route_parity.py` ⇒ `upstream 456 | local 474 | baseline 458 | implemented 388 real + 3 placeholder = 391 / 456 | known_gap 65 | unclaimed 0 | regression 0 | local_only 9`、`gaps by owner: M9=33 M3+=16 M3=11 M10=5` ⇒ **与 base 的九个数逐字相同**，其「0 路由」自报成立 ✓
2. **⑨**：三个门输入 blob 与 base **逐一恒等** —— `report.json` = `3eb0430a…`、`crates/mc-conformance` tree = `f696e5cc…`、`docs/fixtures` tree = `5e44cecd…` ⇒ 「blob 未动」成立 ✓
3. **⑩**：在它的树上 `scanned **1147**` / `baseline 10` / `violations 0`（base = **1130**）——🔴 **增量算得出来**：`git diff --diff-filter=A 067329b6 effc01cb` = **17 个文件、17 个全是 `.rs`** 且全在 ⑩ 的口径 `crates/**.rs` 内 ⇒ `1130 + 17 = 1147` **逐字吻合**；最长新文件 = `wecom_channel/inbound.rs` **773 行**（< 800）✓

⇒ 该片**已具备交付形态**（提交 + 树干净 + 门禁跑过 + 自报可复核），差「推分支 + 开 PR」这一步。

### §130.3 ⑦ / ⑩ 当场；⑨ 静态继承（第 **9** 次）

- **⑦ PASS（1s）**：`upstream 456 (f41fae6b08fb) | local 474 registered | baseline 458 | implemented 388 real + 3 placeholder = 391 / 456 | known_gap 65 | unclaimed 0 | regression 0 | local_only 9`；`gaps by owner = {M9:33, M3+:16, M3:11, M10:5}`（和 = 65 ✓）⇒ **与 §128 / §129 逐字相同**（独立复现，第三轮连测）。
- **⑩ PASS（0s）**：`scanned **1130** / baseline 10 / violations 0` ⇒ 与 §129 的 `1130` 相同（**没有**重犯 §128 抄过期值的错）。
- **⑨ 不冷编（第 9 次生效）**：三个门输入与 §129 的验证树 `aa6dbd64` **逐一恒等**（同上三个 blob/tree）；`git diff --name-only aa6dbd64 067329b6` = 11 个文件，其中 `crates/mc-conformance/**` 与 `docs/fixtures/**` **零命中** ⇒ 读数继承：totals **`fixtures 365 / pass 14 / mismatch 23 / unmounted 22 / placeholder 0 / unevaluable 306`**（直接读 `report.json` 的 `totals`，未跑二进制）。
- 口径提醒（沿用 §112.3）：中间那 8 个 `crates/mc-*/tests/**.rs` 与 `docs/32`、`docs/48` 是 **PR #112** 的落地（测试侧修复），**不进 ⑨ 的三个输入**；`⑦` 的九个数也**不受测试侧影响**。

### §130.4 槽位与派发：**零派发**（空位 1，但无处可派 —— 逐条给可观测判据）

- 空位 = 1（§130.1）⇒ 按纪律**先找候选**，再逐条验阻塞；本轮**仍无一可取**：

| 候选 | 硬前置的可观测判据 | 当轮实测 |
| --- | --- | --- |
| `LUM-1785`（M7-20，backlog，rev 7） | M7-19 的 5 个产物在 base：`git cat-file -e HEAD:crates/mc-channel/src/wecom/{wecom_channel,resolvers,inbox_message,markdown,seal}.rs` | base 上**五条全 MISSING**（在 `effc01cb`、不在 `067329b6`）⇒ **未满足** |
| `LUM-1786`（M7-21 INT，backlog，rev 8） | 前两片（1784 / 1785）**全合** | 1784 未推分支、1785 未起手 ⇒ **未满足** |
| `LUM-1815`（M9-0 anchor，backlog） | `docs/62` §7.1「**M7 全合** + M8 全合」 | M7 未全合（`owners.M7 = 0` 只说明**缺口清空**，不等于 M7-19/20/21 落地）⇒ **未满足** |
| `LUM-1825`（M9-10 INT，backlog）等 M9-1..9 | 全部等 M9-0 | 同上 ⇒ **未满足** |
| `LUM-1745`（M5-D8） | 是否已交付 | `apps/mc-server/src/webhook_worker.rs` 在 base、`main.rs` 第 10 步已接线、issue 停 `in_review` ⇒ **已交付**，不是派发候选 |
| 其余 `todo`（`LUM-2012` / `1835` / `1826` / `1810` / `1805` / `1748` / `1740` / `1737` / `1726` / `1533` / `1521`） | — | **全是 cycle 类 issue**（标题即「multica-rs」或「NN cycle」），非工作片；只登记不动状态 |

- ⇒ **空位刻意留空**。零空位不放空 ⇒ 把两片的**递补读数**刷新到当轮（见 §130.7）。
- **`LUM-1784` 一合入，空位链立刻变为**：1784 合 ⇒ 派 **`LUM-1785`**（`wecom/mod.rs` 的写者链只有它俩，**必须串行**）⇒ 1785 合 ⇒ 派 **`LUM-1786`**（唯一一次 `--write-baseline`）⇒ 之后 M9-0（`LUM-1815`）。

### §130.5 回收 = **+40 MB**（PG 死库 2 个；`df` 粒度看不出，量只认 PG 侧读数）

- **动作**：`pg_stat_activity` 零连接 ∧ 名字里的 run-id 属**已终态且已合入**的片 ⇒ `DROP DATABASE mc_lum1980` / `mc_lum1980f`（都是 `LUM-1980` 的遗留，其 PR #112 已于 23:2xZ 合入、run 终态）。
- **读数**：PG 侧 **6 个库 / ≈84 MB → 4 个库 / 44 MB（含系统库）**；`df --output=avail -m /` = **20068 MiB**（前后同一读数 ⇒ 40 MB 低于本轮 `df` 的可分辨差，**不虚报**）。
- **为什么本轮只有这一步**：唯一的大块（`LUM-1784` 的 `target/` **18G**）是**活物**（正在跑 ⑥）；其余 16 个历史 workdir 各 ~30M（平台管理）；`.repos` 跨项目裸库镜像只登记不删。⇒ 磁盘健康（20G / 58%）时**不做外科手术**（§129.6 lesson 2 的排序键仍适用）。
- **预告**：`LUM-1784` 一合 ⇒ 其 **18G** `target/` 四判据齐即可整删（下一轮的最大单块）。

### §130.6 🔴 门 ⑥ 重定基线（§129.8 的预告项）：**拿到「受控对照对」+ 名录完整性第三次确证**

§129.8 写着「`LUM-1980` 合入后门 ⑥ 的『概率门』旧名录不再是有效参照，需重定基线」。本轮把**近窗口的每一个红 run 逐字取证**（而不是抄结论）：

| run | head | 结论 | 红点（逐字） | 与 `LUM-1980` 名录的关系 |
| --- | --- | --- | --- | --- |
| #365 | `7eb95caf` | **failure** | `telegram.rs:156` `telegram::binding_redeem_is_idempotent_and_classifies_three_failures` **+** `telegram.rs:273` `telegram::install_list_revoke_and_reinstall`（`25 passed; 2 failed`） | **两条都在名录**（`telegram.rs` 两处正是名录里的） |
| #368 | `9f298f0a` | **failure** | `mc-scheduler/tests/jobs_plugin_hook.rs:427` `real_db_the_loop_delivers_each_schedule_bucket_exactly_once`（`0 passed; 1 failed`） | **名录**（**弱等待类**，§128 新抓、后被 `LUM-1980` 并入） |
| #369 | `f3c92913` | **failure** | `telegram.rs:156` 同一条（`26 passed; 1 failed`） | **名录** |
| **#370** | **`0273b817`** | **success** | — | **无修复的树也能绿** |
| #372 | `b248e02b` | cancelled | —（被后推的 #373 顶掉） | 见下 |
| #373 | `067329b6` | in_progress（23:28:36Z 起） | — | **含 8 条修复的第一棵树** |

- 🔴 **受控对照对（本轮最强的新证据）**：`f3c92913` 与 `0273b817` 的差异**只有 `docs/37-M3-W3C-PREFLIGHT.md` 一个文件（`+76 −0`，零代码）**，而 #369（前者）**红**、#370（后者）**绿** ⇒ **「同一棵可编译代码树、红绿交替」拿到第 2 次取证**（第 1 次是 §116 的「同一代码树红绿交替 + 三次红三个不同用例轮换」），且这次是**相邻 run 的严格对照**（点对点、无第三个变量）。
- **名录完整性第三次确证**：近 **3 个红 run** 的**每一个红点**（4 个 panic 点、3 个不同测试函数）**全部**落在 `LUM-1980` 交付的 8 条名录内 ⇒ **窗口内无「未登记签名」**（§128 抓的两条已被它自己并入，本轮复现为 §130 的 #368）。
- **新基线（从 #373 起算）**：`067329b6` 是**第一个含 8 条修复的树** ⇒ 门 ⑥ 的旧名录（`telegram*` / `composio/**` / `jobs_plugin_hook:427` / `lease_db:273`）**从此只作「若再红则判本片回归」的参照**，而**不再**作为「CI 红属正常」的理由。⚠️ **样本量 1**：`#373` 单绿**不足以**宣布概率门关闭（§116 的对照说明无修复的树也能绿）⇒ 需 ≥3 个连续 run 的表态才能改判。
- **顺带的口径**：`#372`（`b248e02b`，**合并提交**）被判 **cancelled** —— 因为 1 分钟后 `067329b6` 又推了一次，GitHub 的 concurrency 把它顶掉。⇒ **「head CI 3/3 绿」这条证据路对合并提交可能天然取不到**（第 3 次出现：`#367` / `#372`），判据链的 ④ 必须保留「合并树同树 `--with-db` 10/10」这条**离线**路。

### §130.7 零空位不放空：两片递补读数刷新到当轮（`LUM-1785` rev 6 → **7**、`LUM-1786` rev 8）

- 内容 = 当轮 ⑦/⑨/⑩ 实测（§130.3，三个数**逐字不变**）+ **写者链的下限换成了「已提交」读数**（更稳）：`crates/mc-channel/src/wecom/mod.rs` base = **81 行 / 21 个 `pub mod`**、`LUM-1784` 的**已提交**树 `effc01cb` = **106 行 / 26 个 `pub mod`**（§129 记的 106/26 是它**工作树**的中间态，这回是提交态 ⇒ 同一个数，但证据等级更高）。
- 号段复核（`docs/32` 当轮实测末四节）：`## 33.` M7-16 / `## 34.` M7-17 / `## 35.` M7-18 / **`## 36.` `LUM-1980`（门 ⑥ 竞态，已落地）** ⇒ M7-19 合并时重编号为 **`## 37.`**、M7-20 = **`## 38.`**、M7-21 INT = **`## 39.`**（rev 6/7 的裁决**当轮复核仍成立**）。

### §130.8 合并预演（给下一轮的判据链 ③ 一条**预置预测**）：**唯一冲突面 = `docs/32`**

- 命令：`git merge-tree --write-tree --messages 067329b6 effc01cb` ⇒ **`151d8da7d23c7dd4d7a59883f454f4df82d79a8e`**，输出里**恰好一次** `CONFLICT (content)`，`--name-only` 也只有**一个文件**：**`docs/32-M3-DAEMON-FACE.md`**。
- ⇒ 除号段外**全自动合并**（含 `crates/mc-channel/src/wecom/mod.rs` —— 两侧加的是**不同行段**，不冲突）；`docs/32` 的冲突正是 §129.10 判定的**号段撞车**（两侧都在文件末尾追加 `## 36.`）。
- **解法（沿用 §127 定式「两侧都保留 + 号段归位到编号单调位」）**：保留 base 的 `## 36. 门 ⑥ 的测试侧竞态收口（LUM-1980）`，把 1784 那节改成 `## 37. M7-19（LUM-1784）：wecom 入站与解析`（含正文里对它的自引用）。该指令**已在 `LUM-1784` 描述 rev 7 的勘误节里**（它自己会在并 base 时解掉；它若不自解，则合并方按上式解）。
- ⚠️ 预演的 `151d8da7` 是**带冲突标记的树**，**不是**落地树预测值（它只用来证明「冲突面 = 1 个文件」这一条）；真预测值要等它推分支后按判据链 ③ 重取。

### §130.9 lesson

1. 🔴 **`multica repo checkout` 的落点必须在起手第一步实测**（§130.1 的坑）：它可能落在**与 `feat/multica-rs-initial` 无共同祖先**的 `main` 线上，而 ⑦/⑩/⑨ **都不会报错**（脚本只看工作树）⇒ 整轮读数会对着**错误的树**。判据：`git rev-parse HEAD` == `git ls-remote origin feat/multica-rs-initial`（不等就先 `checkout -B … origin/feat/…`）。
2. 🔴 **「重定基线」的正确做法是逐字取证，而不是换一句话写结论**：本轮把 3 个红 run 的**失败测试名 + 行号 + passed/failed 计数**从**作业日志**里抠出来（`actions/jobs/<id>/logs`），才拿到「100% 落在名录内」和「受控对照对」这两条**可判真伪**的陈述。⇒ 门 ⑥ 这类**概率门**的证据单位是「**红点签名**」，不是「run 的结论」。
3. 🔴 **受控对照对（只差 docs 的两个 head，一红一绿）是「概率门」最省也最硬的证据形态** —— 它把「CI 玄学」压缩成一条**零代码差异**，且天然排除了「代码回归」这一竞争解释。产出成本 ≈ 2 次 API 读 + 2 次 `git diff --name-only`。
4. **在飞片的自报要「零成本可复核」地验**：⑦（纯 Python 扫源码，1s）、⑨（`git rev-parse <tree>:<path>` 比 blob）、⑩（`scanned` 的**增量可以算出来**：`--diff-filter=A` 的 `.rs` 计数）三项都不需要 cargo，本轮三中三。⇒ **把「自报可复核项」作为验收期望写进片描述**（`--with-db 10/10` 那种必须靠**同树**论证，见 §129 的时间戳法）。
5. **合并预演（`merge-tree --name-only`）应当在片**交付前**就跑一次**：它把「会不会有冲突、冲突在哪个文件」变成一条**前置预测**，本轮据此确认「唯一冲突面 = `docs/32`（号段）」而其它（含共用的 `mod.rs`）**都不冲突** —— 这直接回答了「1785 是否必须等 1784」这类串行化问题（要等，是因为**语义**上的写者链，**不是**因为文本冲突）。

### §130.10 观察项（第 **67** 轮）

- 起手 **无并发 cycle（连续第 20 轮）**；daemon `running_task_count = 2` **未越限**；autopilot 护栏（「同项目存在未终态 cycle issue 时不建新单」）**仍未落地**。
- 积压 `todo` cycle **只登记、不动状态**（本轮不动任何别 issue 的状态；`LUM-1784` 本身 `in_progress` 不重写）。
- 看板精确计数**取不到**（`issue list` 分页上限）⇒ 不把这种读数当判据（沿用 §128.8）。
- 🔴 `docs/32` 末号 = **`## 36.`**（`LUM-1980` 已占）；`## 37.` / `## 38.` / `## 39.` 分别由 M7-19 / M7-20 / M7-21 INT 按**落地顺序**取（已同步进三片 rev）。

### §130.11 next cycle 起点（08:00 / 00:00Z）

- **base = `067329b6` + 本 §130 docs-only 直推**（起手一律 `git rev-parse` 实测，别抄本行）；GH **0 open PR**；daemon **2**（cycle 自身 ∥ `LUM-1784`）。
- **起手第一动作**：① `df -h /`（本轮 20G ⇒ 若 < 4G 先回收）② `git rev-parse HEAD` **对** `git ls-remote origin feat/multica-rs-initial`（§130.9 lesson 1）③ 认证 GH `pulls?state=open` ④ 逐 PID 扫 `/proc/*/cwd`（先读 `cmdline` 是不是 `pi`）⑤ 逐片「run 终态 ∧ 形态判定」，**终态才进判据链**。
- **判据链铁律（沿用 §129.8）**：① 预检 `merge-base..head` numstat == PR API 逐字 ② 形态两半（`merge-base == base` ∧ 尾提交 docs-only）⇒ 真合 + 在片自己的热 target 上重跑 `--with-db` 10/10 ③ 三/四读数（`merge-tree --write-tree` == rehearsal `write-tree` == `refs/pull/N/merge^{tree}` == 落地 `^{tree}`）④ 证据两条路任一（head CI 3/3 绿 **或** 合并树同树 `--with-db` 10/10）⑤ API 钉 40 位 sha + `merge_method=merge` ⑥ 落地树逐字 + `git diff` 空。
- **本轮的预置预测（判据链可直接引用）**：`LUM-1784` 的 PR **只会与 `docs/32` 冲突**（§130.8 的 `merge-tree --name-only` 单文件），解法 = 保留 `## 36.`（`LUM-1980`）+ 它改 `## 37.`；它若已自行并入 base（`merge-base == base`）则**无冲突**，直接走真合。
...
### §133 09:00 cycle（`LUM-2117`，01:00Z 触发）：**起手磁盘 2.5 G（95%）⇒ 先回收**；判据链六步**合并 PR #114**（M10 计划片，docs-only，落地 `8895adf4`）；空位 1 派 **M10-0**（`LUM-2102`，硬前置无）；`LUM-1785` **轮内交 PR #115**（判据链 ①②③ 当轮预置预测）

#### §133.1 起手（**先 `df` 再三连** + 逐 PID 拆槽）：**无并发 cycle（连续第 22 轮）**

| 项 | 起手实测 | 判据/动作 |
| --- | --- | --- |
| `df -h /` | **2.5 G 可用 / 95%**（危险值） | 🔴 本轮的**第一动作 = 回收**（§133.6），安排在读了 issue/评论之后、任何派发之前 |
| base | `d67cf066`（= `ls-remote origin feat/multica-rs-initial`，checkout 落 `main` 线的坑**再次实测命中**：`4fc96f30` ⇒ `checkout -B agent/devbox5/22638972ee6c origin/feat/multica-rs-initial`） | 与 §132 收尾值一致 |
| GH `pulls?state=open` | **1**（`#114` `LUM-2099` M10 计划片） | 进判据链（§133.4） |
| daemon | `running_task_count = 3`（cycle ∥ `LUM-1785` ∥ `LUM-2099`） | 逐 PID 扫 `/proc/*/cwd`（先 `cmdline` 后 `cwd`） |
| `/proc` 逐 PID | `46765` = `pi`（`lum-1785-…`）／`59654` = `pi`（`lum-2099-…`，**当轮内消失**）／`52965` = 本 run | `LUM-2099` 的 run 终态判据：**pi 进程消失 + 终态 comment 已发（01:00:07Z）+ porcelain 空 + head 已推远端** ⇒ 四判据齐 |
| PG | 只有 `multica_lum1785`（20 MB，**活物**） | 无死库可回收（本轮回收全在文件面） |

#### §133.2 独立复采（本轮「可重复动作」）：⑦/⑦b/⑩/⑨/硬前置六条 —— 与 §131/§132 **逐字相同**

| 项 | 本轮实测（base `d67cf066`） | 对比 |
| --- | --- | --- |
| ⑦ `route_parity --quiet` | `upstream 456 (commit f41fae6b08fb) \| local 474 registered \| baseline 458` ／ `implemented 388 real + 3 ph = 391 / 456` ／ `known_gap 65` ／ `unclaimed 0` ／ `regression 0` ／ `local_only 9`（`local_only_placeholder 2`） | **逐字相同** ✓ |
| ⑦ `gaps by owner` | `M9=33  M3+=16  M3=11  M10=5`（和 = 65 ✓，**无 `M7` 键**） | 逐字相同 ✓ |
| ⑦b `slash_alias_audit` | 本地实况 `469` 个 upstream-key 字面量 ⇒ `0 defect / 0 warning`（exit 0） | ✓ |
| ⑩ `file_size_check` | `limit=800  scanned=1147  baseline=10  violations=0` | 逐字相同 ✓ |
| ⑨ 三输入 | `report.json` blob `3eb0430a39c8…`（SAME）／`crates/mc-conformance` tree `f696e5cc…`（SAME）／`docs/fixtures` tree `5e44cecd…` | ⇒ totals `365 / 14 / 23 / 22 / 0 / 306` + rates `0.038356 / 0.378378` **继承，不冷编（第 11 次）** |
| 硬前置六条 | `HEAD:crates/mc-channel/src/wecom/{wecom_channel,resolvers,inbox_message,markdown,seal,stream_store}.rs` | **六条全 PRESENT** ✓ |
| 号段 | `docs/32` 末号 = **`## 37.`**（`grep -n '^## [0-9]'` 实测） | ⇒ M7-20 `## 38.` / M7-21 INT `## 39.` ✓ |

#### §133.3 在飞片 `LUM-1785`（M7-20）体检：**一轮之内从「编译迭代段」跨到「已提交已推 + 已交 PR」**

| 判据 | §132（00:30Z） | 本轮（01:00–01:07Z） |
| --- | --- | --- |
| pi 进程 | 活（`46765`） | **活**（同一个 pid，持续 1h+） |
| `porcelain` | 16 项 | **0 项**（全部落成提交） |
| 提交数 | **0** | **1**（`bc4dd270 feat(m7-20): wecom 打字 / 限流 / 去重 / 追踪 —— 三个接缝的落点（0 路由）`） |
| 远端分支 | 无 | **已推**（`refs/heads/agent/devbox5/b224e1bb1b2b` = `bc4dd270`） |
| PR | — | **`#115`**（`pr-body.md` 01:04Z 落盘） |
| ⑩ 预飞（`wc -l` 扫工作树） | `rate_limit.rs` **825** 行（预警） | **最长 738**（`trace.rs`）< 800 ⇒ **预警已解**（`rate_limit.rs` 拆为 447 + `quota.rs` 393 + `tests.rs` 666） |
| 门禁进程 | — | `bash scripts/gates.sh`（pid `54857`）**仍在跑**（交付段） |

⇒ 形态判定：**在飞 / 交付段**（不是静默死亡）。**cycle 不动它的分支**（它自己正在跑门禁；§130.14 的让位纪律）。

#### §133.4 判据链六步：**合并 PR #114**（M10 计划片 `LUM-2099`，docs-only）⇒ 落地 **`8895adf4`**

| 步 | 判据 | 实测 |
| --- | --- | --- |
| ① 预检 | `git diff --numstat <merge-base>...<head>`（按 filename 排 + 转 int）== PR API 逐字 | `docs/64-M10-PLAN.md 859 0` / `docs/fixtures/m10-declared-routes.tsv 48 0` **逐字相同** ✓ |
| ② 形态两半 | `merge-base == PR 声明的 base` ∧ 尾提交 docs-only | `merge-base = c23fcfad` ✓；单提交、2 文件**全在 `docs/`** ✓（base 已前移到 `d67cf066` ⇒ 需真合） |
| ③ 四读数 | `merge-tree --write-tree` == rehearsal `write-tree` == `refs/pull/114/merge^{tree}` == 落地 `^{tree}` | **四个全 = `5c211b5b6d865f4a428ae8ccaf8958fc6065ac92`** ✓ |
| ④ 证据 | head CI 3/3 绿 **或** 同树 `--with-db` 10/10 | head `372530cb` **CI 3/3 success**：`db — postgres:16 + DB e2e` / `fast — fmt / build / clippy / test / file-size` / `contract — route parity + conformance` ✓ |
| ⑤ 合并 | API 钉 40 位 sha + `merge_method=merge` | `sha=372530cb…` ⇒ `merged:true`，落地 `8895adf4e75a564ebfa46545e6e1082b85e68c5b`（父 `d67cf066` + `372530cb`） |
| ⑥ 落地 | 落地树逐字 + `git diff` 空 | `8895adf4^{tree}` = `5c211b5b…`；`git diff 5c211b5b 8895adf4` **空** ✓ |

- **docs-only PR 的代码面证据 = 子树恒等**（比"重跑一遍门禁再逐个数字比"更省更硬）：`git diff --name-only d67cf066 5c211b5b` = **PR 的 2 个文件**；六棵代码子树逐字相同 —— `crates 2e6c39ba` / `apps 5ca446ea` / `scripts 966e40f6` / `contracts bf977bfe` / `migrations 03654746` / `.github a6b5dfd9` ⇒ 门禁结果在合并树上与 base 必然相同。
- **⑨ 的第二个输入当轮「变了」但结论仍然成立（要写清，不许含糊）**：合并后 `docs/fixtures` tree `5e44cecd… → 18372020…`（+1 个新文件）。**但 `crates/mc-conformance` 对 `docs/fixtures` 零读**（`grep -rn 'docs/fixtures' crates/mc-conformance` = **0 命中**；20 处读者全在 `mc-http`/`mc-openapi`/`mc-channel` 的注释与既有用例里）⇒ ⑨ 的 `totals` 仍由「`report.json` blob 恒等 + `crates/mc-conformance` tree 恒等」两张票决定，`365 / 14 / 23 / 22 / 0 / 306` 继承。
- **合并后复采（同轮）**：⑦ 九个数**逐字不变**；⑩ `scanned 1147 / baseline 10 / violations 0` 不变；**⑦b `--declared docs/fixtures/m10-declared-routes.tsv` 首次可跑 ⇒ `declared 5 / dual-form required 0 / single-form 5 / 0 defect`（exit 0）**（该 fixture 随本 PR 落地，§132 时它在 PR 里 ⇒ 文件不存在）。

#### §133.5 槽位裁定：**空位 1 给了 M10-0（`LUM-2102`）** —— 判据取自计划片的分片表，不是推断

**先给"为什么不是留空"的可观测判据**（逐条来自各片**自己的**前置字段，不当场推断）：

| 候选 | 可派？ | 可观测阻塞判据（当轮实测） |
| --- | --- | --- |
| **`LUM-2102`（M10-0 anchor，stage 1）** | **✓** | 其描述「## 硬前置」**逐字**：**「无（可立即起手）。**但**与 `M9-0` 不得同飞（同 `crates/mc-http/src/routes/{mod,mount}.rs` + `Cargo.lock`）」**；`docs/64` §7.1 第 1 条「M10-0 是所有 M10 片的硬前置」⇒ 它一起手就把 stage 2–6 的 13 片解锁 |
| `LUM-1786`（M7-21 INT） | ✗ | 硬前置 = **全波 M7-0…M7-20 已合入 base** ⇒ 当轮 `#115` 仅**开 PR**、CI `in_progress` ⇒ 未成立 |
| `LUM-1815`（M9-0 anchor） | ✗ | `docs/62` §7.1 第 1 条 = **M7 全合**（`LUM-1786` 落地）⇒ 未成立（第 2 条 M8 全合**已成立**，PR #100 于 2026-09-25T13:41:18Z 合入） |
| M9 stage 2+（含 `LUM-2116`）/ M10 stage 2+ | ✗ | 「上一 stage 未合不进下一 stage」+ anchor 未起 |
| 跨波掉棒审计片 | ✗ | 无待审项：`owners` 无 `M7` 键、`unclaimed 0`、`regression 0`（⑦ 当场读数） |

**派发凭证**：`assign --to-id 3c6087f9-…` + `--no-start`（记录）→ `status todo`（起 run）⇒ workdir `lum-2102-f33bf54cbfff` 落地、`daemon 2 → 3`、平台自动置 `in_progress`（**第 4 次实测一致**）。

🔴 **本轮新加的两条「不得同轮」约束**（写进 `LUM-2102` 的起手前置，见 §133.9 lesson 4）：

1. **`M10-0` / `M9-0` / `M10-6` 两两不得同轮**（`docs/64` §7.2 已定：共享 `routes/{mod,mount}.rs` 或 `Cargo.lock`）。
2. 🔴 **`M10-0` 与 `LUM-1786` 不得同轮** —— 两者**同写** `docs/fixtures/route-parity-baseline.json` 且**都做 `--write-baseline`**（`M10-0` 是「删 `GET /api/feature-flags` 键」的例外刷新，`LUM-1786` 是 M7 波的整表刷新）。两波各自的文档都只声明了"**本波**唯一一次"，**跨波冲突是本轮现算出来的**（`docs/64` §4.3 第 5 条 vs `LUM-1786` 写集）。

#### §133.6 回收 **14.6 GB**：唯一的大块死物 = `LUM-2099` 的 `target/`（四判据齐 ⇒ 整删）

- **四判据（全部当轮实测）**：① pi 进程消失（`59654` DEAD）② `porcelain` 空 ③ 分支已推远端（`refs/heads/agent/devbox5/4fe8c7050b3c` = `372530cb`）④ PR `#114` 已开。
- **效果**：`df` **2.5 G → 17–18 G 可用**（`/` 95% → 63–64%）。
- **一个字节没动的活物**：`LUM-1785` 的 `target/` = **21 G**（pid 存活 + `gates.sh` 在跑 + 该 workdir 仍在写 `pr-body.md`）。**这是本轮磁盘压力的唯一来源** ⇒ 已列入下一轮最高优先级回收项（§133.8）。
- 其余死物量级不足：已完成片的 workdir 除 `target/` 外每个 26–130 MB（含 `LUM-1784` 126 MB / `LUM-2099` 254 MB）；PG 仅 110 MB；`.repos` 1.8 G 是**跨项目裸库**（不动）。

#### §133.7 观察项（第 **69** 轮）

- **看板（项目内 290）**：`in_review 249 / backlog 26 / todo 12 / in_progress 3 / blocked 0`。
  - `backlog 26` = 12（旧）+ 15（`LUM-2099` 当轮新建的 M10×14 + M9 尾账 ×1）− 1（`LUM-2102` 已晋升）✓ **账对得上**。
  - `todo 12` 全是积压的 autopilot cycle 单（`1521/1533/1726/1737/1740/1748/1805/1810/1826/1835/2012`）+ **本单 `LUM-2117`** ⇒ 只登记、不动状态。
- **autopilot 建单护栏仍未落地（第 32 轮）**：积压 `todo` cycle 已 **11 条**（不含本单）。
- **`in_progress` 与 daemon 逐字吻合（第 4 轮）**：目视 `in_progress = 2`（`LUM-1785` + `LUM-2102`）时 daemon = 3 ⇒ 差额**恰好是本 cycle 自身**（本单起手在 `todo`，已按纪律置 `in_progress`；置位后 3 = 3）✓。
- **`LUM-2099` 的交付形态值得记一笔**：它把「终波计划片」做成了 **1 PR + 15 个子 issue + 1 条计划外尾账 issue**（`LUM-2116` M9-11 cloud-runtime 11 条，parent `LUM-1814`）—— 后者是它在自己写集之外**主动补的"不许无主 owner"**项（`docs/64` §9.10）。

#### §133.8 next cycle 起点（09:30 / 01:30Z）

- **base**（本轮收尾）= `8895adf4` + 本 §133 docs-only 直推（起手一律 `git rev-parse` 实测）；GH **1 open PR（`#115`）**；daemon **3/3** = 本 cycle ∥ `LUM-1785`（交付段，`gates.sh` 在跑）∥ `LUM-2102`（M10-0，起手）。
- **第一动作（同 §132.8，但把 `df` 提到最前）**：① `df -h /` **连采两次** ② `git rev-parse HEAD` 对 `git ls-remote`（checkout 落 `main` 线的坑**本轮再次命中**）③ 认证 GH `pulls?state=open` ④ 逐 PID 扫 `/proc/*/cwd` ⑤ 逐片「run 终态（`porcelain` 空 + pi 进程消失 + 已推/已开 PR）∧ 形态判定」。
- **判据链：`PR #115` 的 ①②③ 已当轮预置，下一轮只需补 ④⑤⑥**：
  - ① 预检 **逐字已验**（按 filename 排 + 转 int）：**24 文件 / +7498 / −12**，与 PR API 完全一致（**注意排序键**：`sort` 按整行会把 `1`/`10` 排到 `323` 前面 ⇒ 必须 `awk` 抽出 filename 作首字段再排）。
  - ② 形态：`merge-base = d67cf066` ≠ base ⇒ **真合**（head 尾提交是代码提交，不是 docs-only ⇒ ④ 不能走「子树恒等」捷径）。
  - ③ **四读数预测 = `839b797933c8164d6e8c7426bc2ee82582553517`**（当轮已实测两者相等：`git merge-tree --write-tree 8895adf4 bc4dd270` == `refs/pull/115/merge^{tree}`）。
  - ④ 当轮状态：CI 三条 `in_progress`（`01:05:51Z` 起跑，`mergeable_state = unstable`）⇒ **本轮不等待**（不 `watch`、不睡眠轮询），交下一轮取现成结果；若届时 CI 仍红，走「同树 `--with-db` 10/10」（真库当轮新建）。
- **槽位链（勿抄旧行，起手重算）**：`LUM-1785` 终 ⇒ 判据链合入 `#115`（预期 ⑦ 九个数**逐字不变**——0 路由，但 `scanned` 下限与 `mc-channel` 行数会变）⇒ 空位 ⇒ **先判 `LUM-2102` 是否已终态**：**在飞 ⇒ `LUM-1786` 必须让位**（§133.5 第 2 条：同为 `--write-baseline` 写者）；`LUM-2102` 终 ⇒ 派 **`LUM-1786`**（rev 9 已就绪）⇒ 之后 **`LUM-1815`**（M9-0 anchor，两硬前置届时全成立）。
- 🔴 **回收预告（下一轮最高优先级）**：`LUM-1785` 的 `target/` **21 G** —— 合入 + run 终态 + `/proc` 零命中 + `porcelain` 空 ⇒ **整删**（§131.4 的四判据配方）。**它正在与 `LUM-2102` 的冷建抢盘**（本轮剩 17 G，`gates.sh` 在跑）；若 `LUM-2102` 报 `ENOSPC`，按 `docs/37` §131.4 的「每个 stem 只留最新 mtime」配方先止血，再等 `1785` 整删。

#### §133.9 lesson

1. 🔴 **起手第一动作必须是 `df`（先于三连）**：本轮起手 **2.5 G / 95%** —— 三连都是只读、不会炸，但**任何派发都会踩 ENOSPC**。⇒ 顺序纪律：**`df` → 回收（死物四判据）→ 三连 → 拆槽 → 派发**。回收的排序键仍按「是否碰活物」：死物整删（进程消失 + 已推 + PR 已开 + `porcelain` 空），活物**一个字节不动**。
2. **「空位有货」的判据要从各片自己的前置字段取**：本轮能立刻派 `M10-0`，靠的是它描述里逐字的「硬前置：无（可立即起手）」+ `docs/64` §7.1 的表；`LUM-1786` / `LUM-1815` 的阻塞同理都在它们自己的描述里。⇒ **空位枚举第 1 步 = 打开在飞/待派片的「硬前置」字段**（不是翻自己的记忆、不是推断）。四类候选（递补代码片 / 跨波掉棒审计片 / 下一波计划片 / 显式留空，§132.9）本轮第 1 类**首次真正有货**。
3. **docs-only PR 的证据用「子树恒等」代替重跑门禁**：`git diff --name-only <base> <merge-tree>` == PR 文件清单（全在 `docs/`）+ 六棵代码子树 `rev-parse` 逐一相同 ⇒ 合并树上的门禁结果**必然**与 base 相同。比"重跑 + 逐个数字比对"更省更硬（同 §130 的「同树证据比时间戳硬」）。**但**：这条捷径的前提是**改动全在门禁不扫的目录**（`docs/`、`contracts/`、`deploy/` 都不扫）；代码 PR 不适用。
4. 🔴 **新 finding：`--write-baseline` 的写者清单没有集中登记处，跨波冲突只能现算**。`docs/64` §4.3 第 5 条写「`route-parity-baseline.json` **只被 M10-9 写**（除 M10-0 的删键例外）」，`LUM-1786`（M7-21 INT）的写集里也有同一个文件 + `--write-baseline` ⇒ **两份文档各自都只声明了"本波唯一"，跨波却是双写者**。⇒ 纪律：**审基线写者要看"文件名 × 全仓窗口"，不是看单波的自我声明**；判据 = `grep -rl 'write-baseline' docs/*.md` 交叉各片写集字段。**建议**（下一次改 `docs/64`/`docs/60` 时落实）：把基线文件的写者做成**全仓唯一一张表**（谁、第几波、何时）。
5. **在飞片的 ⑩ 预算是"逐时点"的量**：`rate_limit.rs` 从 §132 的 825 行（未 `git add` 阶段的预警）到本轮落地后的 **447 行**（拆成 `+quota.rs 393 + tests.rs 666`）—— 同一条预警在两个时点的答案完全相反 ⇒ 体检这个数必须**当轮 `wc -l` 工作树**（第 4 次记）。

## §134 09:30 cycle（`LUM-2122`，01:30Z 触发）：**起手 `df` 4.0G / 92% ⇒ 先回收 20.7G**；判据链合并 **PR #115（M7-20 wecom 打字/限流/去重/追踪，0 路由）** → 同轮真合合并 **PR #116（M10-0 anchor）**；空位 2 ⇒ 派 **`LUM-1786`（M7-21 INT）∥ `LUM-2108`（M10-6 bench）**；回收 **41.4G**；🔴 **门 ⑨ 的 ENOSPC 伪装（第 4 次实证）**

### §134.1 起手（`df` 先于三连，§133.9 第 1 条的纪律首次被验证）

- **`df -h /` = 4.0G / 92%** ⇒ 按纪律「`df` → 回收（死物四判据）→ 三连」先动手：`LUM-1785` 的 `target/` **20.7G**（`porcelain 0` + 已推 `bc4dd270` + PR #115 已开 + 无 `cargo`/`rustc` 进程）整删 ⇒ **25.3G**。
- daemon **1**（= 只有 cycle 自身）⇒ **两片（`LUM-1785` / `LUM-2102`）当时都已终态**：前者交付 PR #115、后者交付 PR #116（`multica issue runs` 实测 `completed 01:29:46Z`）。
- base 实测 **`1a5935a2`**（`multica repo checkout` 落 `main` 线的坑**本轮又中**，`git checkout -B <br> origin/feat/multica-rs-initial` 后才是 base）；GH **2 open PR**（#115 / #116）。

### §134.2 判据链 ①–⑥：`PR #115`（M7-20，0 路由）

| 步 | 动作 | 结果 |
|---|---|---|
| ① | 预检 `merge-base..head` `--numstat` 对 PR API（按 filename 排 + 转 int） | **24 文件 / +7498 / −12 逐字相等** |
| ② | 形态判定：`merge-base = d67cf066` ≠ base | 但**前进段 `d67cf066..1a5935a2` 全在 `docs/`**（`docs/37` §133 + `docs/64` + `docs/fixtures/m10-declared-routes.tsv`）⇒ 走「**子树恒等 + CI 三绿**」 |
| ③ | 三/四读数 | `merge-tree --write-tree 1a5935a2 bc4dd270` == rehearsal `write-tree` == **`4d5f16dd`** |
| ④ | head `bc4dd270` 上 CI | **`db` / `contract` / `fast` 三 job 全 `success`**（01:07–01:11Z） |
| ⑤ | API 钉 40 位 sha + `merge_method=merge` | 落地 **`4b9c98a7`** |
| ⑥ | 落地树等式 | `^{tree}` == `4d5f16dd` **逐字** |

- ⑦ 在 base `4b9c98a7` **当场重跑** `--only route-parity,file-size` = **2/2 PASS / 0s**，九个数与 #115 合入前**逐字相同**（0 路由片，符合预期）。

### §134.3 判据链（含真合 + 同树门禁）：`PR #116`（M10-0 anchor）

- 🔴 **同一轮里先合 #115 会当场作废 #116 的 ②**：`refs/pull/116/merge^{tree}` = `390ec1cc` 是对**当轮 base `1a5935a2`** 算的（我实测 `merge-tree --write-tree 1a5935a2 4a7000cc` == `390ec1cc` == GH merge ref，**三项齐**）；但 #115 一落地，`#116` 的 base 前进段就含 **24 个代码文件** ⇒ ② 失效 ⇒ **必须真合 + 在合并树上重跑门禁**。
- **真合**：唯一冲突面 = `docs/32`（号段）⇒ 按**定式解**（两侧都保留 + 编号单调）：`## 38.`（M7-20）+ `## 39.`（M10-0）⇒ 合并提交 **`49d0c22a`**，推回 `agent/devbox5/f33bf54cbfff`（`4a7000cc..49d0c22a`）。
- 门禁 `--with-db`（真库**当轮新建**：角色 `mc_lum2122` 带 `CREATEDB` + 库 `multica_lum2122`）：**首轮 9/10 —— ⑨ 红**，日志判据 `couldn't create a temp dir: No space left on device (os error 28) at …/target/debug/deps/rustcvXB0Zt` ⇒ **ENOSPC 伪装**（第 4 次实证，前三次是 ⑤/⑥/⑧/⑨ 的其它形态）。
- **止血**：删该 workdir 的 `target/debug/incremental`（**18.2G**）⇒ `--only conformance` 复跑 **PASS（51s）**；**两次调用间树哈希恒等**（`30efc0ea` 前后一致）⇒ 合并证据记为 **10/10 on tree `30efc0ea`**。
- 合并树读数：⑦ `local 473 / baseline 457 / implemented 388 real + 3 ph = 391 / known_gap 65 / unclaimed 0 / regression 0 / **local_only 8**`（`9 → 8` = M10-0 预删幽灵占位 `GET /api/feature-flags` 的效果）；⑤ `3812 passed / 0 failed`；⑥ `724 passed / 0 failed`（`migrate=0, e2e=0`）。
- 形态：head **已是 base 的祖先**（片自己合过 base）⇒ ② 成立 ⇒ 三哈希 `30efc0ea` == predict == head tree；API 钉 sha → 落地 **`4f11bb7f`**，`^{tree}` == `30efc0ea` **逐字**。
- ⇒ **base（本轮收尾）= `4f11bb7f`**；再回收 `LUM-2102` 的死 `target/` **20.7G**（`porcelain 0` + run `completed`）⇒ **本轮合计回收 41.4G**。

### §134.4 派发（空位 2；两片写集零交集）

- ⑦ on base：`owners = {M9:33, M3+:16, M3:11, M10:5}`（和 = 65 ✓）；`M7` 键**不存在**（§127 已收口）。
- **slot 1 = `LUM-1786`（M7-21 INT，rev 9 → 14）**：全波**唯一** `--write-baseline`。起手补充 rev 10 + 勘误 rev 11 逐条落地：**片前基线 = `457`**（不是 458：M10-0 的删键型最小刷新已随 #116 进 base）、⑨ blob 订正为 **`3eb0430a…`**（rev 10 误写 `fa53d084`，那是 #111 **合入前**的旧值）、号段取 **`## 40.`**（`## 38.`/`## 39.` 本轮已落地）。
- **slot 2 = `LUM-2108`（M10-6 性能 bench，0 路由，rev 1 → 4）**：无硬前置、**不刷任何快照**（⑦/⑨/⑩ 全不动）⇒ 与 1786 的写集（`docs/32`、`docs/37`、`docs/60 §11`、`docs/fixtures/route-parity-baseline.json`）**零交集**（`docs/fixtures/` 下不同文件）。
  - 🔴 **选它的理由 = 它是 M10 全波唯一改 `Cargo.lock` 的片**，而它的死对头 **`M9-0`（`LUM-1815`）的硬前置（M7 全合 + M8 全合）本轮已全部成立 ⇒ `M9-0` 下一轮就可能起手** ⇒ **本轮是 M10-6 最干净的窗口**（`docs/64` §7.2 的硬规则：`M10-0`/`M9-0`/`M10-6` 两两不得同轮）。
- 派发顺序（照 `docs/60` §7.4 的纪律）：`update --description-file` → `assign --to-id … --no-start` → `status todo`；daemon **1 → 3/3**（cycle + 1786 + 2108）；看板 `in_progress 3` 与 daemon **逐字吻合**。

### §134.5 号段（`docs/32`）

- 已占至 `## 39.`（`## 36.`=`LUM-1980`、`## 37.`=M7-19、`## 38.`=M7-20、`## 39.`=M10-0）⇒ **下一片若开新节取 `## 40.`**（号由**落地顺序**占）。

### §134.6 ⑦ / ⑨ / ⑩ 当轮读数（base `4f11bb7f`，全部当场实测）

- ⑦ `upstream 456 (commit f41fae6b08fb) | local 473 registered | baseline 457 | implemented 388 real + 3 placeholder = 391 / 456 | known_gap 65 | unclaimed 0 | regression 0 | local_only 8`；不变式 `implemented + known_gap == 456` ✓。
- ⑨ 三个门输入（可作「是否需冷编」的判据）：`crates/mc-conformance/report.json` blob **`3eb0430a39c8cf6c50ccafe626542abba3eaa0d4`**、`docs/fixtures` tree `f8a22c4edded82a1da8dfe120afe29f68ebe39ba`、`crates/mc-conformance` tree `f696e5ccd520631d66b55c145f4c42bc15e43ed7` ⇒ totals `365 / 14 / 23 / 22 / 0 / 306`（待冷编核对）。
- ⑩ `file_size_check: limit=800 scanned=1170 baseline=10 violations=0`。

### §134.7 next cycle 起点（10:00 / 02:00Z）

- **base** = `4f11bb7f` + 本 §134 docs-only 直推（起手一律 `git rev-parse` 实测）；GH **0 open PR**；daemon **3/3** = cycle ∥ `LUM-1786`（`lum-1786-c95b99fea078`）∥ `LUM-2108`（`lum-2108-3a8eabef12b3`）；磁盘 **36.9G**（回收后）。
- **第一动作（顺序不可换）**：① `df -h /` 连采两次（在飞冷建 ≈7G/片）② `git rev-parse` 对 `git ls-remote` ③ 认证 GH `pulls?state=open` ④ **从 `/` 起手**逐 PID 扫 `/proc/*/cwd`（先读 `cmdline` 是不是 `pi`）⑤ 逐片「run 终态（`porcelain` 空 + pi 消失 + 已推/已开 PR）∧ 形态判定」。
- **槽位链（勿抄旧行）**：
  - `LUM-1786` 终 ⇒ 判据链合入。⚠️ 它是**基线写者** ⇒ 合并后**必须当场重跑 ⑦** 复核 `baseline == 片后 local 实测`、并核对 ⑨ totals 未位移；合入后 **`LUM-1815`（M9-0）的两个硬前置全部成立** ⇒ 派 **M9-0 单独跑**（anchor 不并行；它写 `state.rs`/`mount.rs` ⇒ 若 `LUM-2108` 仍在飞则 `Cargo.lock` 争用 ⇒ **`M10-6` 必须先终态/合入**）。
  - `LUM-2108` 终 ⇒ 递补 **`LUM-2103`/`LUM-2104`/`LUM-2105`（M10-1/2/3，stage 2）**（硬前置 M10-0 已合；**加路由片 ⇒ 不得与 `--write-baseline` 片同轮**）。
- **⑨ 归属**：M10 的 `/api/config` 17 条 + `/health` 1 条（= 18 条）一旦挂上路由就会从 `unmounted` 变可判定 ⇒ 归 **M10-INT（`LUM-2111`）**，**普通片不许顺手刷** `report.json`。
- **三个基线写者不得同轮**：`LUM-1786`（M7-21 INT）/ `LUM-1825`（M9-INT）/ `LUM-2111`（M10-INT）。

### §134.8 lesson

1. 🔴 **「四读数」里的 GH merge ref 只对「它被算出来的那个 base」有效**：`refs/pull/N/merge^{tree}` 是 GitHub 按**当时的 base tip** 算的 —— 本轮 #115 的 merge ref = `839b7979`（对旧 base `8895adf4`），而我对**当前 base** 的 `merge-tree --write-tree` = `4d5f16dd`；两者**都合法但不可互证**。⇒ 纪律：④ 那一项**自己算**，GH 的 merge ref 只作旁证，且**先 `git rev-parse refs/pull/N/merge^1` 确认它的 base 是谁**（§133.8 的「预测 `839b7979`」当轮复核**成立但已过期**）。
2. 🔴 **ENOSPC 伪装门红第 4 次实证（这次是 ⑨，形态 = 在 `target/debug/deps` 建临时目录）**：判据仍是日志 `os error 28` / `No space left on device`。**确证配方 = 「删 `incremental` ⇒ 单跑该门转绿」+「两次调用间树哈希恒等」**（比 `df` 瞬时读数硬）。本轮删 **18.2G** 即 PASS。
...
## §135 M7-21 INT（`LUM-1786`，base `dd0e8c51`）：M7 波收口的口径表（⑦ 基线片前值的三类成因 / ⑨ 归属 / 掉棒成因）

> 本节是 M7 波（W7 渠道面）**收口**的口径收口，与 `docs/32` §40、`docs/60` §11 三处互指。
> 本片 **0 路由 / 0 代码**；起手 base = `dd0e8c51`；唯一写 `docs/fixtures/route-parity-baseline.json` 的片。

### §135.1 ⑦ 的「片前基线」是一个**只能在起手那一刻取**的读数 —— 它有三类过期成因

M7-21 的 DoD 逐字要求「`--write-baseline` 的**片前值** = 当轮 `baseline` 字段的**实测值**」。这一波里这个值被写进 issue 描述**五次都对、五次都过期**（`344` → `406` → `430` → `458` → `457`）。三类成因，逐类给判据：

| 成因 | 实例（本波实测） | 判据 / 纪律 |
| --- | --- | --- |
| **① 时间流逝**（别的波在中间合入并刷了基线） | `344`（计划期）→ `406`（M6-INT） | 凡"计划期写的绝对读数"一律作废：它连**派发那一刻**都不是真的 |
| **② 同一波里另一片的合法副作用** | `406 → 458`（**M8-7 INT** `LUM-1804` 的整树刷新，2026-09-25 13:18，`f3e9794e`） | 判据 = `git merge-base --is-ancestor <该片提交> <上次刷新提交>`；真 ⇒ 它的位移是**合法**的，不是你算错了 |
| **③ 同一文件被"删键型最小刷新"动过** | `458 → 457`（**M10-0** 预删幽灵占位 `GET /api/feature-flags`，只删 1 键、**不**整树重写） | 判据 = 基线文件的**净增为 −1** 且 `local` 同步 −1 ⇒ 是**有意的例外**（`docs/32` §39.2 第 4 条给的授权），不是漂移 |

⇒ **纪律（写进后续任何 INT 片的起手清单）**：`--write-baseline` 的「片前」**不许从文档/issue 里抄**，必须由**当轮** `python3 scripts/route_parity.py` 的 `baseline` 字段现场取；**片后**同理必须是当轮 `local` 的实测值。

### §135.2 `local` 是**未折叠的注册点**数，不是"路由条数"

`local 473` 里 `/x` 与 `/x/` **各算一条**（`route_parity.py` 在**比较**时折叠、在**计数**时不折叠）。本波实测：`local 473` 里有 **74 组**双形态键（`slash_aliases` 数组 / `note:` 行逐条列出：`473 − 74 = 399` 个折叠键）。⇒ **推论两条**：① 不许按"折叠后的键集合"去推算 `local`（`docs/37` §95 的 M6-INT 先例）；② 一个 0 路由片的 `local` **必须逐字不变**（本波 15 个 0 路由片全部逐字命中）。

### §135.3 「M7 全绿」≠「渠道面全绿」—— 渠道面的三层账

| 层 | 判据 | M7 的终态 |
| --- | --- | --- |
| **路由层**（456 条账内） | ⑦ `owners` 里没有 `M7` 键 | **✓ 全绿**（24/24 落地，`M7-4/5/9/14/15` 五个有路由片） |
| **渠道面读侧**（账外） | 上游 `GET /api/chat/history` 的 owner 是 **M4** | M4 侧**已落地**；M7 只供数据 ⇒ **R-M7-6 闭** |
| **入站/长连接层**（**完全不在 456 条账内**，`docs/60` §1.5） | 生产宿主是否真的起了连接 | **✗ 未接**（`docs/32` §40.6 **D-1**：`main.rs:184` 的 `deps` 硬编码 `None`，5 个 `register_with` 与 `Supervisor::spawn` 生产调用点 **0**） |

⇒ **口径**：`owners.M7 = 0` 只说明**账内**全绿。渠道面的**主证据**（§4.2 的"每渠道至少一条真实收发回路"）在**账外**，必须按 §40.7 的**逐文件取证**读，不能拿 ⑦ 当它的替身。

### §135.4 「未配置」语义**按端点而异**，且 wecom 那一格是 **403 不是 503**

* `docs/60` §8 R-M7-3 与上游 `router.go:917` 的注释**都写 wecom 端点 503** —— **两处都不是代码**。逐字实现是 `writeFeatureDisabled`（`handler.go:578` ⇒ `http.StatusForbidden`）⇒ **3 条路由回 403 `wecom_not_configured`**；503 只属于**够不着 `WeCom`**（凭据没验过）的那一格。取证与用例见 `docs/32` §31.2 **D4**。
* lark 的列表是 **200 空 + `install_supported:false`**（不是 403/503）。⇒ 纪律：**禁止"统一 503"**；每条路由的未配置语义必须**逐 fixture**对齐（`docs/60` §2.4）。

### §135.5 单副本部署契约（`R-M7-1` 的正面那一半）

本仓**没有 Redis** ⇒ 上游 4 处跨副本协调（WS 租约 CAS / 入站去重 / 安装会话 / 出站重投递）全部换成**进程内替身**。**部署契约 = 单副本**，或"渠道连接只在一个副本上开"。⇒ 后续任何"为什么多副本下同一个 installation 被连两次"的提问，答案在这一条（`docs/32` §40.3 R-M7-1 + §31/§33/§34/§38 的 R1）。

### §135.6 本波**掉棒**的成因口径（第 17 类）：**接线点落在已冻结的写集里**

本波抓到 **2 项**掉棒，**同一族**（`docs/32` §40.6）：

* **D-1**：五渠道的**生产装配**。裁定分散在 7 片（§13.5/§17.4/§18.4/§19.4/§24.4/§31/§36.4）的"归宿主"一句里，而"宿主"= `apps/mc-server/src/channels.rs` = **M7-0 的 anchor 写集**（§3.1 明写"其余片只读"）⇒ **没有任何片**被授权回头改它。
* **D-2**：wecom 的**凭据探针接线**。产物（真传输）是 M7-16 的写集，接线那一行在 **M7-15 的写集** ⇒ 两片都不具备改动对方的授权。

⇒ **成因口径**：当一条裁定把工作切成「产物在 A 片、接线在 B 片」而 **B 片已经合入**时，**"归宿主""由后续片接线""INT 复核"三种写法都不算 owner**。
**判据化动作（写给 cycle / 派发者，花 30 秒）**：写"归 X"之前，`grep` 一下那个逐字路径**是不是某个已合入片的写集**；是 ⇒ 连同"谁有权改"一起写进**新开片的描述**（`docs/57` §9.8.3 的执行规则 1、2）。
**"复核"不是交付**：本片被指派"复核该端点转绿"，实测结论是**未转绿** —— 把复核的**否定结论**如实登记（`docs/32` §40.4 末条）比"看起来完成了"重要。

### §135.7 ⑨ 的归属口径（本波只核对、不动快照）

* `crates/mc-conformance/report.json` 的刷新**只归该波的 INT 片**，且**只在有 `unmounted` ⇒ 可判定 的位移时才刷新**。
* 本波实测：**M7 面 8 条 `unmounted → pass` 已由 M7-14（PR #111）自己刷掉**（`pass 7 → 14` / `unmounted 29 → 22`）；**wecom 面 0 条 fixture**（阴性对照组）⇒ M7-INT **一格未动**，只核对（`report matches`）。
* **下一批已知位移归 M10-INT**：`/api/config` 17 条 + `/health` 1 条 = 18 条 `unmounted`。**普通片不许顺手刷**（`docs/32` §39.8 R-M10-4 同款纪律）。
* **先例对照**：M8-6 的 composio 一条**自己刷了**（`unmounted → pass`）；M7-9 挂 7 条路由**没刷**（它那 5 条 fixture 是 `unevaluable`，actor 非 anonymous）⇒ **判据是 fixture 的 `actor` 与 `via`，不是"这一片加了几条路由"**。

### §135.8 门 ⑥ 的新基线（承 §130.6，本轮复核后仍成立）
