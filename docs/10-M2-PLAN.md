...
> 本节是 **2026-09-22 18:00 的手工对账快照**，保留作历史记录。自 **LUM-1376** 起，对账已脚本化为
> 一条命令：`python3 scripts/route_parity.py`（两侧数据与命令语义见 `docs/22-ROUTE-PARITY.md`；
> 复现本节数字请重跑该命令，不要手工再抄一遍）。**未认领盲区**（某条上游路由谁都没认领）与
> **已实现路由丢失**（基线里有、源码里没了）都会让该命令以 `1` 退出；两者共享的 owner 看板在 §4.3。
