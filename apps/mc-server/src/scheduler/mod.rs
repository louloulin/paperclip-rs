//! M5-9（`LUM-1659`）：把 `mc-scheduler` 的**内核 + 三个 job**接进 `multica-server`。
//!
//! # 这一片解决什么问题
//!
...
//! 2. 三个缺失的**数据面端口**生产实现（[`schedule_port`] / [`wakeup_port`] /
//!    M6-8 加的 [`hook_port`]）；
//! 3. [`start`]：装配 4 个端口 + `register_all` + `spawn()`，返回 `SchedulerHandle`
...
pub use hook_port::McPluginHookPort;
...
/// 装配（不启动）：注册三个 job，返回还没 `spawn` 的 `Manager`。
///
/// **顺序有语义**：`register_all` 必须在 `Manager::spawn` 之前（`spawn` 消费 `self`，
/// 注册表在那一刻冻结）。这个函数也是真库用例的缝：测试可以断言 `manager.jobs()`
/// 里确实有三个 job（注册是纯内存的，不需要数据库），再自己 `spawn()` 观察租约行。
///
/// M6-8（`LUM-1673`）在这里**只加了一个实参**（`hook_port`）：hook job 的数据面就是
/// `mc-http` 的 hook 引擎，因此 `apps/mc-server` 不需要多一条依赖边（`mc-http` 早已在）。
pub fn build(db: &Db, hub: Arc<Hub>, options: Options) -> anyhow::Result<Manager> {
    ...
    let ports = JobPorts::new(
        ...
        // M6-8：hook 的定时投递（`plugin_hook_schedule_dispatch`）。
        Arc::new(McPluginHookPort::new(db)),
    );
